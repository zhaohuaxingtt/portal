;(function (global, factory) {
    ...
    var oldLanguage = {
        ...
        SPR_FRM_XGYSPJ_JSHWC: 'Approval Completed', //家审核完成
        ...
        SPR_FRM_XGYSPJ_GYSZRPGMO_JBXX:
            'Supplier Entry Evaluation Model - Basic Information', //供应商准入评估模型-基本信息
        ...
        SPR_FRM_XGYSPJ_GDDGSDZCQK: 'Shareholder Support', //股东对公司的支持情况
        SPR_FRM_XGYSPJ_GDZZZJHM: 'Shareholder ID ', //股东证照/证件号码
        SPR_FRM_XGYSPJ_DGLX: 'Ownership Class (Natural Person or Legal Person)', //股东类型
        SPR_FRM_XGYSPJ_CGBL: 'Shareholding ratio', //持股比例（%）
        SPR_FRM_XGYSPJ_GYSZRPGMO_CWXX:
            'Supplier Entry Evaluation Model - Finanical Situation', //供应商准入评估模型-财务状况
        SPR_FRM_XGYSPJ_TBQ: '填补前（元）', //填补前（元）
        SPR_FRM_XGYSPJ_GYSZRPGMX_DFZB:
            'Supplier Entry Evaluation Model - Rating Criteria', //供应商准入评估模型-得分指标
        SPR_FRM_XGYSPJ_ZB: 'Rating Criteria', //指标
        SPR_FRM_XGYSPJ_DF: 'Grades', //得分
        ...
        SPR_FRM_XGYSPJ_BGZRZHSG:
            'Strike/Disaster/Accident: Current year -10; Last year -0;<br/> Quality Problem: General -5; Serious -10;<br/> Call back: Possibility/Small scale -5; Large scale (countrywide or worldwide) -10;<br/> Multiple statements, external reports on financial problems/Lack of capital: -5;<br/> Administrative penalty (including environmental penalty): General -5; Serious -10;<br/> Supply delayed or ceased due to operating insufficiency, financial difficulty, etc.: -10;<br/> Supply delayed or ceased due to unknown reasons: -5;', // 罢工/自然灾害/事故：发生当年-10，次年不扣分； 质量问题：一般-5；严重-10； 召回：可能发生/小规模-5；大规模（全球或多地区）-10； 多套报表、其他人反馈有财务问题/资金紧张：-5； 行政处罚（包括环保处罚）：一般-5，重大-10； 因资金、不能持续经营等问题供货延迟或停供：-10； 原因尚未确定的供货延迟或停供：-5； 内部管理不善（送样延迟、送样质量不好、拖欠工资等）：-5；
        SPR_FRM_XGYSPJ_YFXXHYJJ:
            'Solved without further impacts;<br/> Having no impacts on operating and financing activities, eg. chattel mortgage;<br/> Not in compliance with facts or overstatd, eg. multiple statements with special reasons;<br/> Already reflected in financial statements, similar to signals in preliminary rating, eg. consecutive losses.', //原风险信号已解决，无后续影响； 不影响经营、财务，如动产抵押； 核实后，风险信号情况与事实不符或夸大，如多套报表存在特殊原因； 财务问题会直接反映在报表（资产负债表、利润表）中，与初评模型中 指标类似的信号（如连年亏损），不额外扣分；
        ...
        SPR_FRM_CBPJ_SXZTYWQCXQZ: '所选状态有误，请重新选择。', //所选状态有误，请重新选择。
        ...
        EKL_YJGL_XZYJJCTIP: 'please edit the basic information first, and then select the file to upload',
...
//
    ...
    xmlHttp.open("GET", "http://10.122.18.136:8088/i18n/getTranslationMap?from=protal", false);
