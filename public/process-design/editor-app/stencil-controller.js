...
angular.module('activitiModeler').controller('StencilController', [
  ...
  function ($rootScope, $scope, $http, $modal, $timeout) {
    ...
    $scope.propertyWindowState.toggle = function () {
      $scope.propertyWindowState.collapsed =
        !$scope.propertyWindowState.collapsed
      $timeout(function () {
    ...
    $scope.editorFactory.promise.then(function () {
      window.parent.postMessage(
        JSON.stringify({ key: 'designReady', value: true }),
        '*'
      )
      ...
      $http({ method: 'GET', url: KISBPM.URL.getStencilSet() })
        .success(function (data, status, headers, config) {
          ...
          for (
            ...
            // 如果属性有tab切换
            if (data.stencils[stencilIndex].tab) {
              $scope.tabsMap[data.stencils[stencilIndex].id] = {
                tabs: data.stencils[stencilIndex].tab,
                activeTab: data.stencils[stencilIndex].tab[0].tabName
              }
            }
            ...
            if (!removed) {
              if (quickMenuDefinition.indexOf(stencilItem.id) >= 0) {
                quickMenuItems[quickMenuDefinition.indexOf(stencilItem.id)] =
                  stencilItem
              }
            }
          ...
          for (var i = 0; i < quickMenuItems.length; i++) {
            if (quickMenuItems[i]) {
              availableQuickMenuItems[availableQuickMenuItems.length] =
                quickMenuItems[i]
            }
          }
        ...
        .error(function (data, status, headers, config) {
          console.log(
            'Something went wrong when fetching stencil items:' +
              JSON.stringify(data)
          )
        })
      ...
      $scope.editor.registerOnEvent(
        ...
        function (event) {
          ...
          if (shapes && shapes.length > 0) {
            ...
            if (
              $rootScope.selectedElementBeforeScrolling &&
              $rootScope.selectedElementBeforeScrolling.getId() ===
                selectedShape.getId()
            ) {
            ...
            // selectedItem.tabs = stencil.packages();
            for (var i = 0; i < properties.length; i++) {
              ...
              var propertyConfig =
                KISBPM.PROPERTY_CONFIG[key + '-' + property.type()]
              ...
              if (propertyConfig === undefined || propertyConfig === null) {
                ...
                if (
                  KISBPM.CONFIG.showRemovedProperties == false &&
                  property.isHidden()
                ) {
                  continue
                }
            ...
            var stencilId = stencil.id() && stencil.id().split('#')[1]
            if ($scope.tabsMap[stencilId]) {
              selectedItem.tabs = $scope.tabsMap[stencilId].tabs
              selectedItem.activeTab = $scope.tabsMap[stencilId].activeTab
            }
        ...
        function (event) {
          ...
          if (shapes && shapes.length == 1) {
            ...
            if (additionalIEZoom === 1) {
              absoluteXY.y =
                absoluteXY.y - jQuery('#canvasSection').offset().top + 5
              absoluteXY.x =
            ...
            var stencilItem = $scope.getStencilItemById(
              selectedShape.getStencil().idWithoutNs()
            )
            var morphShapes = []
          ...
          if (shapes && shapes.length == 1) {
            ...
            var showSelectShapeDialog = function () {
              ...
              $modal({
                ...
                keyboard: true,
                template:
                  'editor-app/popups/select-shape.html?version=' + Date.now()
              })
            }
      ...
      $scope.quickAddItem = function (newItemId) {
        $scope.safeApply(function () {
          ...
          if (shapes && shapes.length == 1) {
            ...
            var option = {
              type:
                $scope.currentSelectedShape.getStencil().namespace() +
                newItemId,
      ...
      if ($scope.dragCanContain) {
        ...
        if ($scope.quickMenu) {
          ...
          if (shapes && shapes.length == 1) {
            ...
            var option = {}
            option.type =
              currentSelectedShape.getStencil().namespace() + ui.draggable[0].id
      ...
      if ($scope.dragModeOver != false) {
        ...
        if (aShapes.length == 1 && aShapes[0] instanceof ORYX.Core.Canvas) {
          ...
          if (item.type === 'node') {
            ...
            var parentItem = $scope.getStencilItemById(
              parentCandidate.getStencil().idWithoutNs()
            )
            if (parentItem.roles.indexOf('Activity') > -1) {
              if (
                item.roles.indexOf('IntermediateEventOnActivityBoundary') > -1
              ) {
                _canContain = true
              }
            ...
            var targetStencil = $scope.getStencilItemById(
              parentCandidate.getStencil().idWithoutNs()
            )
            if (targetStencil) {
    ...
    $scope.toggleTab = function (tabName) {
      $scope.selectedItem.activeTab = tabName
    }
    $scope.selectedItemTabProperties = function () {
      var tabProperties = []
      if ($scope.selectedItem && $scope.selectedItem.tabs) {
        $scope.selectedItem.tabs.forEach(function (e) {
          if ($scope.selectedItem.activeTab === e.tabName) {
            tabProperties = e.packages.map(function (pack) {
              return 'oryx-' + pack.replace('package', '')
            })
          }
        })
      }
      const useroveridIndex = tabProperties.indexOf('oryx-useroverrideid')
      if (useroveridIndex > -1) {
        tabProperties[useroveridIndex] = 'oryx-overrideid'
      }

      if ($scope.selectedItem && $scope.selectedItem.properties) {
        var properties = []
        $scope.selectedItem.properties.forEach(function (e, index) {
          if (tabProperties.indexOf(e.key) > -1) {
            e['index'] = index
            properties.push(e)
          }
        })
...
KISBPM.CreateCommand = ORYX.Core.Command.extend({
    ...
    if (this.currentReference && this.position) {
      if (this.shape instanceof ORYX.Core.Edge) {
        if (!(this.currentReference instanceof ORYX.Core.Canvas)) {
          ...
          if (
            this.currentReference.getStencil().idWithoutNs() ===
            'TextAnnotation'
          ) {
            var midpoint = {}
            midpoint.x = 0
            ...
            this.shape.dockers
              .last()
              .setReferencePoint(this.currentReference.bounds.midPoint())
          }
        } else {
      ...
      if (containedStencil.defaultAlign() === 'north') {
        ...
        pos.x -=
          bc.width() / 2 + ORYX.CONFIG.SHAPEMENU_CREATE_OFFSET + bs.width() / 2
  ...
  rollback: function () {
    ...
    this.facade.setSelection(
      this.facade.getSelection().without(this.shape, this.edge)
    )
  }
})
