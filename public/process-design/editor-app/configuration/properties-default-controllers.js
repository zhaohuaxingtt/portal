...
var KisBpmStringPropertyCtrl = [
  ...
  function($scope) {
    ...
    /* initValue()
    function initValue() {
      if ($scope.property.package === 'documentationpackage') {
        if ($scope.tabsMap && $scope.tabsMap.userTask) {
          $scope.disabled = true
        }
      }
    } */
...
var KisBpmBooleanPropertyCtrl = [
  '$scope',
  function($scope) {
    console.log('bool', typeof $scope.property.value)
...
var KisBpmTextPropertyCtrl = [
  ...
  function($scope, $modal) {
    var opts = {
      template:
        'editor-app/configuration/properties/text-popup.html?version=' +
        Date.now(),
      scope: $scope
    }
