    ...
    new Ajax.Request(source, {
      ...
      onSuccess: function(result) {
        ...
        $A(plugin).each(function(node) {
          ...
          var url =
            ORYX.PATH + ORYX.CONFIG.PLUGINS_FOLDER + pluginData['source']
      ...
      if (jsonProp.items && jsonProp.items instanceof Array) {
        jsonProp.items.each(
          function(jsonItem) {
            ...
            ] = new ORYX.Core.StencilSet.PropertyItem(jsonItem, namespace, this)
          }.bind(this)
        )
      } else {
      ...
      if (jsonRules.layoutRules) {
        ...
        jsonRules.layoutRules.each(
          function(rules) {
            ...
            if (rules['ins']) {
              layoutRules[layoutKey]['ins'] = (rules['ins'] || []).map(function(
                e
              ) {
...
ORYX.Core.StencilSet.StencilSet = Clazz.extend({
  ...
  construct: function(source, modelMetaData, editorId) {
    ...
    if (ORYX.CONFIG.BACKEND_SWITCH) {
      ...
      new Ajax.Request(
        '/portal/process-design/editor-app/i18n/stencilset.json',
        {
          asynchronous: false,
          method: 'get',
...
ORYX.Editor = {
  ...
  unregisterOnEvent: function(eventType, callback) {
    if (this.DOMEventListeners.keys().member(eventType)) {
      this.DOMEventListeners[eventType] = this.DOMEventListeners[
        eventType
      ].without(callback)
    } else {
              ...
              if (property.type() == ORYX.CONFIG.TYPE_CHOICE) {
                  ...
                  function(item) {
                    item.refToView().each(
                      function(itemRef) {
                        ...
                        if (
                          !refreshedSvgElements[svgElem.id] ||
                          prop == item.value()
                        ) {
                ...
                property.refToView().each(
                  function(ref) {
                    ...
                    if (
                      property.type() === ORYX.CONFIG.TYPE_KISBPM_MULTIINSTANCE
                    ) {
