...
const requstSupplier = axios(process.env.VUE_APP_NEWS)
...
//查询最新新闻列表
export function findLatestNews(params) {
  return requstSupplier({
    url: `/newsQueryService/findLatestNews`,
    method: 'POST',
    params:{supplier:params,userId: store.state.permission.userInfo.id}
  })
}

//查询最热新闻列表
export function findHotNews(params) {
  return requstSupplier({
