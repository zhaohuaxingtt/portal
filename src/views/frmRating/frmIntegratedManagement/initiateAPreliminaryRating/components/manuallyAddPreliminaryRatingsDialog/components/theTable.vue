  ...
  <div>
    <div class="margin-bottom20 clearFloat">
      <div class="floatright">
        <!-- 导出供应商模板-->
        <iButton @click="exportSupplierTemplate" :loading="exportButtonLoading">{{ $t('SPR_FRM_FRMGL_DCGYSMB') }}</iButton>
        <!-- 导入供应商-->
        <importButton
        ...
        />
        <!--添加至初评-->
        <iButton @click="addToPreliminaryReview">{{ $t('SPR_FRM_FRMGL_TJZCP') }}</iButton>
        <!--删除-->
        <el-popconfirm
...
export default {
  ...
  methods: {
    ...
    async addToPreliminaryReview() {
      ...
      const oldSupplierIdList = [...this.$store.state.frmRating.addToPreliminaryReviewList];
      const newSupplierIdList = Array.from(new Set(oldSupplierIdList.concat(supplierIdList)));
