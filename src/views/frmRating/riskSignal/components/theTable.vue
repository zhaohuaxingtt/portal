  ...
  <iCard class="table-card">
    <div class="margin-bottom20 clearFloat">
      <div class="floatright">
        <!-- 复制-->
        <iButton
          v-permission="SUPPLIER_FRMRATING_RISKSIGNAL_FUZHI"
          @click="handleView('copy')"
          >{{ $t('SPR_FRM_FXXH_FZ') }}</iButton
        >
        <!-- 新建-->
        <iButton
          @click="handleView('creat')"
          v-permission="SUPPLIER_FRMRATING_RISKSIGNAL_XINJAIN"
          >{{ $t('SPR_FRM_FXXH_XJ') }}</iButton
        >
        <!-- 删除-->
        <iButton
          @click="handleDelete"
          v-permission="SUPPLIER_FRMRATING_RISKSIGNAL_SHANCHU"
    ...
    >
      ...
      <template #oldRiskSignalId="scope">
        <span
          v-if="scope.row.oldRiskSignalId"
          class="cursor openLinkText"
          @click="handleView('view', { id: scope.row.oldRiskSignalId })"
          >{{ $t('LK_CHAKAN') }}</span
        >
      </template>
      <template #furtherRatingResult="scope">
        <icon
          :name="
            scope.row.furtherRatingResult === '红'
              ? 'iconhongdeng'
              : scope.row.furtherRatingResult === '绿'
              ? 'iconlvdeng'
              : scope.row.furtherRatingResult === '黄'
              ? 'iconhuangdeng'
              : ''
          "
          symbol
...
import {
  getRiskSignal,
  riskDelete
} from '@/api/frmRating/riskSignal/riskSignal.js'
      ...
      if (flag === 'creat') {
        ...
        if (this.selectTableData.length === 1) {
          if (this.selectTableData[0].status === 'MANAGEMENT') {
            this.$router.push({
              path: '/supplier/frmrating/riskSignal/details',
              query: { id: this.selectTableData[0].id, flag: flag }
            })
          } else {
