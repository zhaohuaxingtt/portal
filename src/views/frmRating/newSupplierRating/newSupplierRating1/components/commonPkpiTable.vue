...
<template>
  <el-form :model="{tableData}" status-icon :rules="rules" ref="commonTableForm">
    <el-table :height="height" tooltip-effect='light' :data='tableData' :empty-text="$t('LK_ZANWUSHUJU')" v-loading='tableLoading' @selection-change="handleSelectionChange" :row-class-name="handleTableRow">
      <el-table-column v-if="selection" type='selection' width="50" align='center'></el-table-column>
      <el-table-column v-if='index' type='index' width='50' align='center' label='#'></el-table-column>
      <template v-for="(items,index) in tableTitle">
        <!--说明-->
        <el-table-column :width="items.width" :show-overflow-tooltip='items.tooltip' :key="index" align='center' v-if="items.props === 'description'" :prop="items.props" :label="items.key ? $t(items.key) : items.name">
          <template #header>
            ...
            <el-popover trigger="hover" placement="top-start">
              <div class="tip-box">
                ...
                <div class="tip-content" v-html="$t('SPR_FRM_XGYSPJ_YFXXHYJJ')"></div>
              </div>
