...
<template>
	<div>
		<!-- SFRM综合评价 -->
		<iCard title="SFRM综合评价" collapse class="margin-top20" isRequired>
			<iInput  :disabled="isDisabled" type='textarea' :autosize='rowRange'  v-model="info.sfrmOverallMerit"></iInput>
		</iCard>
		<!-- 深入评级结果 -->
		<iCard  title="深入评级结果" collapse class="margin-top20" isRequired>
		...
		<!-- 后续跟踪建议 -->
		<iCard title="后续跟踪建议" collapse class="margin-top20" isRequired>
			<div class="margin-bottom20">
				<div class="flex-align-center">
					<span class="nowIndustry">后续跟踪频率</span>
					<iSelect :disabled="isDisabled" v-model="info.trackFrequencyAgain">
						<el-option value="0" key="0" label="无需后续跟踪"></el-option>
						<el-option v-for="num in range(1,36)" :key="num" :value="num" :label="num + '月'"></el-option>
					</iSelect>
					<span class="margin-left70">1-36个月/无需后续跟踪</span>
				</div>
			</div>
			<div class="title">补充建议</div>
			<iInput :disabled="isDisabled" type='textarea' :autosize='rowRange' placeholder="请输入" v-model="info.addAdvice"></iInput>
		</iCard>
		<!-- 背景 -->
		<iCard title="背景" collapse class="margin-top20">
			<iInput :disabled="isDisabled" type='textarea' :autosize='rowRange' placeholder="请输入" v-model="info.supplementarySuggestions"></iInput>
		</iCard>
