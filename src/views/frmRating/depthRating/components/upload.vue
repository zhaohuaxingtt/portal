<template>
	<!--附件上传-->
	<iDialog :title="$t('SUPPLIER_FUJIANSHANGCHUAN')" :visible.sync="value" width="90%" @close="clearDiolog">
		<div class="content">
			<div class="flex-align-center navBtn">
				<!--删除-->
				<iButton class="margin-right10" @click="depAttachmentDel">{{ $t('LK_SHANCHU') }}</iButton>
				<!--附件上传-->
				<el-upload
					class="upload"
			...
			<tableList :tableData="tableListData" :tableTitle="tableTitle" :tableLoading="tableLoading" :index="true"
				...
				<template #name="scope">
					<span @click="downFile(scope.row.name)" class="down">{{scope.row.name}}</span>
				</template>
			</tableList>
