<template>
	<!--加入集团-->
	<iDialog :title="$t('SPR_FRM_DEP_ADDGROUP')" :visible.sync="value" width="90%" @close="clearDiolog" destroy-on-close>
		<div class="content">
			<div class="flex-between-center margin-bottom20">
				<div class="flex-align-center">
					<span class="nowIndustry">{{$t('SPR_FRM_DEP_SELECTGROUP')}}</span>
					<iSelect :placeholder="$t('LK_QINGXUANZE')" v-model="group" value-key='id' @change="getTableList">
						<el-option :value="item" :label="item.deepCommentSupplierName" v-for="item in financialSuppleData" :key="item.id"></el-option>	
					</iSelect>
				</div>
				<div class="flex-align-center">
					<!-- 提交 -->
