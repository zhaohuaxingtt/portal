<template>
	<!--提交清单审批-->
	<iDialog :title="$t('SPR_FRM_DEP_SUBMITQDSP')" :visible.sync="value" width="90%" @close="clearDiolog">
		<div class="content">
			...
			<el-table ref="multipleTable" :data="tableListData" style="width: 100%;margin-bottom: 20px;" row-key="id"
				border default-expand-all :tree-props="{children: 'children'}" @selection-change="handleSelectionChange"
				@select="checkChildren" @select-all="checkAll">
				<el-table-column type="selection" width="55" :selectable="checkSelectable">
				</el-table-column>
				<el-table-column prop="myindex" align='center' label='#' width='50'>
				</el-table-column>
				<el-table-column :label="$t('LK_GYSMCJT')" show-overflow-tooltip>
					<template slot-scope="scope">
						{{scope.row.nameZh||scope.row.financialSupplementName}}
					</template>
				</el-table-column>
				<el-table-column v-for="item,i in tableTitle1" :key='i' :prop="item.props" :label="$t(item.key)">
				</el-table-column>
				<el-table-column :prop="deepCommentReasons" :label="$t('SPR_FRM_DEP_DEPREASON')">
					<template slot-scope="scope">
						<iSelect v-model="scope.row.deepCommentReasons" :disabled="disabled">
							<el-option :value="item.name" :label="item.name"
								v-for="item in fromGroup.DEEP_COMMENT_REASON" :key="item.code"></el-option>
						</iSelect>
					</template>
				</el-table-column>
				<el-table-column :prop="deepCommentReasons" :label="$t('SPR_FRM_DEP_XXYY')">
					<template slot-scope="scope">
						<iInput v-if="scope.row.deepCommentReasons=='其他原因'" :disabled="disabled"
							v-model="scope.row.deepCommentOtherReasons" @change="check2(scope.row)" maxlength="6"></iInput>
					</template>
				</el-table-column>
				...
				<el-table-column :prop="noDeepCommentReason" :label="$t('SPR_FRM_DEP_BPJYY')">
					<template slot-scope="scope">
						<iInput v-if="scope.row.children!==undefined && scope.row.isDeepComment=='0'"
							v-model="scope.row.noDeepCommentReason" @change="check(scope.row)"></iInput>
					</template>
				</el-table-column>
			</el-table>
		</div>
