...
<template>
	<div>
		<!-- SFRM综合评价 -->
		<iCard title="SFRM Overall evaluation" collapse class="margin-top20" isRequired>
			<iInput  :disabled="isDisabled" type='textarea' :autosize='rowRange' placeholder="Please enter." v-model="info.sfrmOverallMerit"></iInput>
		</iCard>
		<!-- 深入评级结果 -->
		<iCard  title="In-depth analysis result" collapse class="margin-top20" isRequired>
			<tableList :tableData="tableListData" :tableTitle="tableTitle" :selection="false" :index="true" :tableLoading="tableLoading" >
				<template #deepCommentRatingDate> 
					<iDatePicker disabled style="width:120px !important" v-model="info.deepCommentRatingDate" value-format="yyyy-MM-dd"></iDatePicker>
				</template>
				<template #deepCommentRatingResults>
					<el-dropdown :split-button="true" @command="changeGrade">
							<icon symbol style="fontSize:12px" :name="trans(info.deepCommentRatingResults)"></icon>
						<el-dropdown-menu slot="dropdown" v-if="!isDisabled">
							<el-dropdown-item v-for="item in grade" :key="item.id" :value="item.name" :label="item.name" :command="item.name">
								<icon symbol :name="trans(item.name)" ></icon></el-dropdown-item>
						</el-dropdown-menu>
					</el-dropdown>
				</template>
			</tableList>
		...
		<!-- 后续跟踪建议 -->
		<iCard title="Follow-up Suggestions" collapse class="margin-top20" isRequired>
			<div class="margin-bottom20">
				<div class="flex-align-center">
					<span class="nowIndustry">Frequency of Follow-up</span>
					<iSelect :disabled="isDisabled" v-model="info.trackFrequencyAgain" placeholder="Please select">
						<el-option value="0" key="0" :label="$t('SPR_FRM_DEP_NOFOLLOW')"></el-option>
						<el-option v-for="num in range(1,36)" :key="num" :value="num" :label="num + 'months'"></el-option>
					</iSelect>
					<span class="margin-left70">1-36 months/No follow-up</span>
				</div>
			</div>
			<div class="title">Supplementary Suggestions</div>
			<iInput :disabled="isDisabled" type='textarea' :autosize='rowRange' placeholder="Please enter." v-model="info.addAdvice"></iInput>
		</iCard>
		<!-- 背景 -->
		<iCard title="Background" collapse class="margin-top20">
			<iInput :disabled="isDisabled" type='textarea' :autosize='rowRange' placeholder="Please enter." v-model="info.supplementarySuggestions"></iInput>
		</iCard>
