...
export default {
  ...
  created() {
    ...
    if (this.defaultChoose) {
      this.selectedTableData = this.defaultChoose
    }
    this.query()
  },
  methods: {
    ...
    handleSelectionChange(val) {
      ...
      console.log('handleSelectionChangehandleSelectionChange', val)
    },
    save() {
      ...
      /* this.$nextTick(() => {
        if (this.selectedTableData && this.selectedTableData.length > 0) {
          const items = this.tableData.filter(
            e => e.id === this.selectedTableData[0].id
          )
          if (items.length > 0) {
            this.$refs.positionTable.toggleRowSelection(items[0], true)
          }
        }
      }) */
    }
