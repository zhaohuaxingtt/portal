<template>
  <div>
    ...
    <iCard class="margin-top20">
      ...
      <iTableCustom
        ref="reportContentTable"
        class="margin-top20"
        :data="tableListData"
        :loading="tableLoading"
        :columns="tableSetting"
        permission-key="ADMIN_REPORT_CONTENT"
...
export default {
  ...
  methods: {
    ...
    async edit(row) {
      if (row.published)
        return this.$message.warning('请先下架，再进行修改操作')
      this.tableLoading = true
      await reportContentDetailById(row.id).then((res) => {
        if (res) {
          res.file = [{ fileName: res.source, fileUrl: res.cover }]
    ...
    del(row) {
      if (row.published)
        return this.$message.warning('请先下架，再进行删除操作')
