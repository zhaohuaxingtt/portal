  ...
  <div class="mtz-select">
    <i-search @sure="sure" @reset="reset">
      <iFormGroup label-position="top">
        ...
        <iFormItem label="有效期起" class="SearchOption">
          <iDatePicker
        ...
        <iFormItem label="有效期止" class="SearchOption">
          <iDatePicker
    ...
    <!-- 一二次件关系有效性确认 -->
    <iDialog
      :visible.sync="isShowRelationalValidity"
      v-if="isShowRelationalValidity"
      title="一二次件关系有效性确认"
      width="80%"
    >
      <div class="relationalValidityTitleBox" slot="title">
        <p>{{ language('YECJGXYXXQR', '一二次件关系有效性确认') }}</p>
        <el-tooltip
          content="以下表格内零件已有一/二次件对应关系，请确认当前生效记录"
          placement="top"
          effect="light"
        >
...
export default {
  ...
  mounted() {
    this.initSearchData()
    queryDeptSectionForPart({}).then((res) => {
      this.departmentDrop = res.data
    }) //初始化科室
  ...
  methods: {
    ...
    handleSource(val) {
      ...
      if (val.source === '0') {
        path =
          '/mtz/annualGeneralBudget/locationChange/MtzLocationPoint/overflow/applyInfor'
        query = {
        ...
        path =
          'aeko/mtz/details?aekoNum=' +
          val.sourceCode +
          '&objectAekoPartId=' +
          val.objectAekoPartId +
          '&status=VIEW'
