<template>
  <div class='mtz-select'>
    ...
    <iCard class="OrganizationTable">
      <div class="export">
        <div class="tt">{{language('SHICHANGJIALAIYUANXIANGQING','市场价来源详情')}}</div>
        <div>
          <uploadButton
          ...
          <iButton v-if="!editMode" @click="handleManualSync">{{language('SHOUGONGTONGBUSHUJU','手工同步数据')}}</iButton> 
          <iButton v-if="!editMode" @click="handleClickUpload">{{language('SHANGCHUAN','上传')}}</iButton> 
          <iButton v-if="!editMode" @click="handleEdit">{{language('BIANJI','编辑')}}</iButton>
          <iButton v-if="!editMode" @click="handleExport">{{language('DAOCHU','导出')}}</iButton> 
      ...
      <div>
        ...
        @handleSelectionChange="handleSelectionChange">
          ...
          <template #marketPriceSourceTypeValue="scope">
            <iSelect
            v-model="scope.row.marketPriceSourceTypeValue"
            v-if="editMode"
            :placeholder="language('QINGXUANZE', '请选择')"
            @change="handleChangePriceSource($event, scope.row)">
              <el-option value="手工上传" :label="language('SHOUGONGSHANGCHUAN', '手工上传')"></el-option>
              <el-option value="系统自动" :label="language('XITONGZIDONG', '系统自动')"></el-option>
            </iSelect>
          ...
          <template #externalMarketPriceSource="scope">
            ...
            :placeholder="language('QINGXUANZE', '请选择')">
              <el-option 
              v-for="(item, index) in dropDownData['dropDownExternalMarketPriceSourceData']" 
              :key="index" 
              :value="item" 
              :label="item">
              </el-option>
          ...
          <template #externalMaterialCode="scope">
            <iInput v-if="editMode" v-model="scope.row.externalMaterialCode" :placeholder="language('QINGSHURU', '请输入')" />
            <p v-if="!editMode">{{scope.row.externalMaterialCode}}</p>
          </template>
          <!-- 取价规则 -->
...
import { fetchTableData, fetchDropDownData, fetchManualSync, fetchExport, fetchUpload, fetchEdit, fetchPriceRule } from '@/api/mtz/database/sourceMarketPrice'
export default {
  components:{
    iSearch,
    iInput,
  ...
  methods:{
          ...
          for(let key in res.data) {
            switch (key) {
              case 'parentMaterialCategoryCodeList':
                // 原材料大类编号
                this.$set(this.dropDownData, 'dropDownParentMaterialCategoryCodeData', res.data[key])
                break;

              case 'externalMarketPriceSourceList':
                // 外部市场价来源
                this.$set(this.dropDownData, 'dropDownExternalMarketPriceSourceData', res.data[key])
                break;

              case 'materialCodeList':
                // 原材料牌号
                this.$set(this.dropDownData, 'dropDownMaterialCodeData', res.data[key])
                break;

              case 'materialElementShortList':
                // 材料大类-元素简称
                this.$set(this.dropDownData, 'dropDownMaterialElementShortData', res.data[key])
                break;

              case 'materialNameList':
                // 牌号名称
                this.$set(this.dropDownData, 'dropDownMaterialNameData', res.data[key])
                break;

              case 'materialNameZhList':
                // 材料中类名称-中文
                this.$set(this.dropDownData, 'dropDownMaterialNameZhData', res.data[key])
                break;
             
              case 'materialCategoryDescList':
                // 材料中类描述
                this.$set(this.dropDownData, 'dropDownMaterialCategoryDescData', res.data[key])
                break;
              default:
                break;
            }
    ...
    // 保存编辑
    handleSave() {
      if(this.tableListData.find(item => item.marketPriceSourceTypeValue == '系统自动' && !item.externalMarketPriceSource)) {
        return iMessage.warn(this.language('QXZWBSCJLY', '请选择外部市场价来源'))
      }
      this.editMode = false
