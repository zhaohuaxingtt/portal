...
<template>
  <iCard>
    ...
    <el-table ref="moviesTable"
      ...
      <el-table-column prop="monthFromTo"
                       show-overflow-tooltip
                       header-align="center"
      ...
      <el-table-column prop="remark"
        ...
        <template slot-scope="scope">
          <el-tooltip class="item"
                      ...
                      placement="top">
            <el-button type="text">{{language('JUPEILIYOU','拒赔理由')}}</el-button>
          </el-tooltip>
        </template>
      </el-table-column>
      <el-table-column prop="fileList"
        ...
        <template slot-scope="scope">
          <iButton type="text"
                   ...
                   @click="openFile(scope.row.fileList)">{{language('FUJIANQINGDAN','附件清单')}}</iButton>
        </template>
      </el-table-column>
    </el-table>
    ...
    <editDetailDialog :key="editModalParams.key"
    ...
    <el-dialog :title="language('FUJIANQINGDAN','附件清单')"
...
export default {
  ...
  methods: {
    ...
    sendSupplier () {
      ...
      if (this.muiltSelectList[0].status === '草稿' || this.muiltSelectList[0].status === '供应商拒绝' || this.muiltSelectList[0].status === '审批不通过') {
        let params = []
    ...
    handleClickEdit () {
      ...
      if (this.muiltSelectList[0].status === '草稿' || this.muiltSelectList[0].status === '供应商拒绝' || this.muiltSelectList[0].status == '审批不通过' || this.muiltSelectList[0].status == '审批退回' || this.muiltSelectList[0].status == 'RISE审批通过' || this.muiltSelectList[0].status == '供应商确认') {
    ...
    submit () {
      ...
      if (this.muiltSelectList[0].status === '供应商确认' || this.muiltSelectList[0].status === '审批退回' || this.muiltSelectList[0].status === 'RISE审批通过') {
        let params = []
        this.muiltSelectList.forEach(item => {
          params.push(item.id)
        })
    ...
    recall () {
      ...
      if (this.muiltSelectList[0].status === '草稿' || this.muiltSelectList[0].status === '供应商确认中' || this.muiltSelectList[0].status === '供应商拒绝' || this.muiltSelectList[0].status === '审批不通过') {
        let params = []
        this.muiltSelectList.forEach(item => {
          params.push(item.id)
        })
        ...
        iMessage.error("'草稿、供应商确认中、供应商拒绝、审批不通过状态可以撤回")
      }
    },
    edit () {
      if (this.muiltSelectList.length === 0) {
