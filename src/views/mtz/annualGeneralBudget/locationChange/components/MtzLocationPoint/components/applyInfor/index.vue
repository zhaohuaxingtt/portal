  ...
  <div>
    <iCard>
        <div slot="header"
            ...
            <div class="opration">
                <iButton @click="edit" v-show="disabled">{{ language('BIANJI', '编辑') }}</iButton>
                ...
                <iButton @click="relation" v-if="applyNumber===''&&showType">{{ language('GLLJDDSQ', '关联零件定点申请') }}</iButton>
                <iButton @click="cancelRelation" v-if="applyNumber!==''&&showType">{{ language('QUXIAOGUANLIAN', '取消关联') }}</iButton>
            </div>
        </div>
        <div class="tabsBoxInfor">
            <div class="inforDiv" v-for="(item,index) in tabsInforList" :key="index">
                <span>{{item.name}}</span>
                <iInput :disabled="item.prop == 'mtzAppId'||item.prop == 'linieName'||item.prop == 'appStatus'||item.prop == 'meetingName'?true:disabled" class="inforText" v-model="inforData[item.prop]" v-if="item.type!=='select'"></iInput>
                <iSelect
                  style="width:68%;"
                  v-else
                  :disabled="disabled"
                  ...
                  >
                    <el-option :value="item.code" :label="item.message" v-for="item in getFlowTypeList" :key="item.code"></el-option>
                </iSelect>
            </div>
        </div>
        <span style="display:block;margin-bottom:20px;">Linie上会备注</span>
        <el-input :disabled="disabled" type="textarea" :rows="4" placeholder="请输入备注" v-model="inforData.linieMeetingMemo"></el-input>
    </iCard>
    <theTabs></theTabs>
