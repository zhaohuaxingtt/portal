<template>
  <i-card v-permission='MTZ_REPORT_DISPLAY_ANNUAL_BUDGET_BRAND_PAGE'>
    ...
    <div class="title-box">
      ...
      <div class="right-box">
        <show-me-components :is-only-myself='queryForm.onlySeeMySelf'
                            @showOnlyMyselfData='showOnlyMyselfData' />
        <year-change-components class="margin-left20"
                                @changeYear='changeYear' />
      </div>
    </div>
    <i-table-custom :loading='loading'
                    class="margin-top20"
      ...
      this.brandMTZTitle = [
        ...
        },
        {
          ...
          customRender: (h, scope) => {
            ...
            return <div title={tooltipContent}>{(Number(scope.row.curPrice) / 1000000).toFixed(2).replace(/(\d{1,3})(?=(\d{3})+(?:$|\.))/g, '$1,')}</div>
          }
        },
        {
          prop: 'lastPrice',
          ...
          customRender: (h, scope,) => {
            ...
            return <span title={tooltipContent}>{(Number(scope.row.lastPrice) / 1000000).toFixed(2).replace(/(\d{1,3})(?=(\d{3})+(?:$|\.))/g, '$1,')}</span>
          }
        }
      ]
    },
