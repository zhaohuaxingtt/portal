<template>
  <iCard>
    <div class="flex justify-between margin-bottom20">
      ...
      <div>
        <iButton @click="dialog.show = true">{{
          language('添加ProD文档')
        }}</iButton>
      </div>
    </div>
    ...
    >
      <div class="content">
        <div class="flex margin-bottom20">
          ...
          />
          <iButton style="margin-left: 10px" @click="queryProDList">{{
            language('搜索')
          }}</iButton>
        </div>
        <iTableCustom
  ...
  methods: {
    async save() {
      if (this.selectList.length == 0)
        return this.$message.warning('请选择文档')
      // 添加文档
      try {
        ...
        let res = await addProcessProds(this.id, this.selectList)
        if (res.error) {
