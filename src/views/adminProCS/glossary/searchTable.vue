<template>
  <el-card class="mt20 style">
    ...
    <div class="flex flex-row justify-end mt20 mb20">
      ...
      <iButton @click="modifyHandler" :disabled="selectedItems.length == 0">{{
        language('修改')
      }}</iButton>
        ...
        if (res) {
          ...
          content.forEach((e) => {
            if (e.attachMents && e.attachMents.length) {
              e.attachMents.forEach((attach) => {
                console.log('attach.url', attach.url)
                if (attach.url && attach.url.startsWith('/uploader/')) {
                  attach.url = attach.url.replace('/uploader/', '')
                }
              })
            }
          })
