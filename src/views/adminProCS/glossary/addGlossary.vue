<template>
  ...
  >
    <el-form
      v-loading="loading"
      ref="form"
      :model="newGlossaryForm"
      :rules="newGlossaryRules"
      label-width="100px"
      class="glossaryForm validate-required-form"
    >
      <iFormItem :label="language('标题')" prop="title">
        <iInput
          v-model="newGlossaryForm.title"
      ...
      <iFormItem :label="language('标题首字母')" prop="firstLetter">
        <iInput
          v-model="newGlossaryForm.firstLetter"
          :placeholder="language('请输入')"
          maxLength="1"
        ></iInput>
      </iFormItem>
      ...
      <iFormItem :label="language('版本号')" prop="version">
        <iInput
          v-model="newGlossaryForm.version"
          :placeholder="language('请输入')"
          maxLength="20"
        ></iInput>
      </iFormItem>
      <iFormItem :label="language('词条内容')" prop="termsContent">
        <iInput
          ...
          :rows="3"
          type="textarea"
          v-model="newGlossaryForm.termsContent"
          :placeholder="language('请输入')"
          maxLength="100"
        ></iInput>
      </iFormItem>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      newGlossaryRules: {
        ...
        publishDate: {
          required: true,
          message: this.language('请选择发布时间'),
          trigger: 'blur'
        },
        termsContent: {
          required: true,
          message: this.language('请输入词条内容'),
          trigger: 'blur'
        }
      },
      fileList: [],
  ...
  methods: {
    ...
    clearFormVal() {
      Object.keys(this.newGlossaryForm).map(
        (key) => (this.newGlossaryForm[key] = '')
      )
    },
    sure() {
      this.$refs.form.validate(async (v) => {
        if (v) {
          try {
            ...
            this.loading = true
            if (this.modifyFlag) {
              await modifyGlossaryById(this.modifyGlossaryId, formData).then(
                (res) => {
                  if (res) {
    ...
    uploadHandle(file) {
      return new Promise((resolve, reject) => {
        ...
        uploadImage(this.modifyGlossaryId, formdata)
          .then((res) => {
            resolve({
