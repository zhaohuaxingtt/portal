      ...
      >
        <el-row class="form-row">
          <iFormItem class="meet-desc">
            ...
            >
              <el-option
                v-for="(item, index) in [{value: resultData.attendeeGroupName, label: resultData.attendeeGroupName}]"
                :key="index"
                :label="item.label"
                :value="item.value"
              >
          ...
          <iFormItem prop="attendees" class="meet-desc" :rules="rule">
            <iInput v-model="resultData.attendees" type="textarea" resize="none" rows="4" />
          </iFormItem>
        </el-row>
        <div>
          <p class="agenda-box-title">
            <span>Agenda items</span>
            <span>(Please click the following tittle which will lead to the detail content.)</span>
          </p>
          <ul class="agenda-item-box">
            <li v-for="(item, index) in resultData.themens" :key='item.id' :class="[choosedIndex == index+1 ? 'active-agenda-item' : '', 'agenda-item']">
              ...
              <div class="agenda-item-content">
                <p class="task">Task</p>
                <iFormItem :prop="'themens.' + index + '.conclusion'" class="meet-desc" :rules="rule">
                  <iInput v-model="item.conclusion" type="textarea" resize="none" rows="4" />
                </iFormItem>
                <p class="task">Result</p>
                <iFormItem :prop="'themens.' + index + '.result'" class="meet-desc" :rules="rule">
                  <iInput v-model="item.result" type="textarea" resize="none" rows="4" />
                </iFormItem>
              </div>
            </li>
          </ul>
