          ...
          <el-col>
            <iFormItem :label="language('零件六位号')" prop="sixPartCode">
              <iInput
                ...
                v-model="formContent.sixPartCode"
                :disabled="inputSta"
                @input="searchlinieBySix"
              ></iInput>
            </iFormItem>
          </el-col>
        </el-row>
        <el-row>
          <el-col>
            <iFormItem :label="language('零件名称(中)')">
              <iInput
                :placeholder="language('请输入')"
                v-model="formContent.partNameZh"
                :disabled="inputSta"
              ></iInput>
            </iFormItem>
          </el-col>
        ...
        <el-row>
          <el-col>
            <iFormItem :label="language('零件名称(德)')">
              <iInput
                :placeholder="language('请输入')"
                v-model="formContent.partNameDe"
                :disabled="inputSta"
              ></iInput>
            </iFormItem>
          </el-col>
  ...
  created() {
    if (this.inputId != '') {
      getSixNUmDetail(this.inputId)
        ...
        .catch((err) => {
          this.$message.error(err.desZh || '获取产品组关联零件6位号详情失败')
        })
    } else {
      this.papgeTitle = '新增'
    }
