<template>
  <iPage>
    ...
    <iDialogPage v-if="openSta" :openStatus="openSta" :inputId="inputId" 
    @closeDialog='closeDialog' @upSearch='getFormContent' :productId='productId'
    ></iDialogPage>
  </iPage>
</template>
...
import {saveProductData,getProductById,delProSixNum} from '@/api/materiel/productData.js'

export default {
  name:'产品主数据详情',
  mixins:[pageMixins],
  components:{iPage,pageHeader,iCard,iFormItem,iInput,iButton,iTableCustom,iDialogPage},
  computed:{},
  data(){
    ...
    return{
      ...
      rules:{
        ...
        pgNameZh:[{required:true,message: '请输入产品组中文名称', trigger: 'blur'}],
        pgNameDe:[{required:true,message: '请输入产品组德文名称', trigger: 'blur'}],
  ...
  methods:{
    ...
    getFormContent(){
      this.loading = true
      getProductById(this.productId).then((val)=>{
        if(val.code == 200){
          let data = val.data
          ...
          this.iniFormContent.pgNameZh = JSON.parse(JSON.stringify(val.data.pgNameZh))
          this.iniFormContent.pgNameDe = JSON.parse(JSON.stringify(val.data.pgNameDe))
          this.iniFormContent.releaseNomiWeek = JSON.parse(JSON.stringify(val.data.releaseNomiWeek))
          this.iniFormContent.nomiToBffWeek = JSON.parse(JSON.stringify(val.data.nomiToBffWeek))
          this.iniFormContent.bfFirstTryoutWeek = JSON.parse(JSON.stringify(val.data.bfFirstTryoutWeek))
          this.iniFormContent.firstTryOtsWeek = JSON.parse(JSON.stringify(val.data.firstTryOtsWeek))
          this.iniFormContent.firstTryEmWeek = JSON.parse(JSON.stringify(val.data.firstTryEmWeek))
    ...
    cancel(){
      ...
      if(JSON.stringify(this.iniFormContent) != JSON.stringify(this.formContent) && this.productId){
