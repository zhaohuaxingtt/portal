<template>
  <el-card v-loading="loading">
    ...
    <iTableCustom
      ...
      :data="extraData.tableListData"
      :columns="tableSetting"
      :extra-data="extraData"
      :permission-key="'ADMIN_PROCS_PROB_moduleManagement_' + type"
  ...
  methods: {
    ...
    editHandler() {
      if (this.selectionRowList.length) {
        this.isEdit = true
        this.extraData.selectionRowIds = this.selectionRowList.map(
          (item) => item.menuId
        )
    ...
    async confirmEditHandler() {
      ...
      for (let i = 0; i < list.length; i++) {
        list[i].adminUserName = list[i].adminUserId
          .map((e) => this.extraData.nameListKV[+e].nameZh)
          .join(',')
        list[i].adminUserId = list[i].adminUserId
          ? list[i].adminUserId.join(',')
          : ''
        if (
          this.extraData.selectionRowIds.includes(list[i].menuId) &&
          !list[i].adminUserId
        ) {
          f = list[i]
          break
