  ...
  <div class="rightContent ml20" v-loading="loading">
    <div class="top-box">
      ...
      <img
        ...
        @click="search"
      />
      <iButton class="ask-btn" @click="handleQuestion">{{
        language('我要提问')
      }}</iButton>
    </div>
    <div class="mt15 line"></div>
    <div class="title">
      {{ currMoudleName ? language(`${currMoudleName}`) : '' }}
    </div>
    <div class="line"></div>
...
export default {
  ...
  methods: {
    ...
    search() {
      this.content = this.changeColor()
      this.$nextTick(() => {
        document.querySelectorAll('.searchActive')[0] &&
      ...
      if (this.searchKey) {
        ...
        Array.from(temp.childNodes).forEach((e) => {
          ...
          if (innerText.includes(this.searchKey)) {
            e.innerHTML = innerText.replace(
              new RegExp(this.searchKey, 'g'),
              '<a id="seach" class="searchActive" style="color:red">' +
                this.searchKey +
                '</a>'
            )
          }
