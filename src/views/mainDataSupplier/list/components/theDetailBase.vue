  ...
  <iCard :title="language('基本信息')"
    ...
    <el-form label-position="left"
      ...
      <el-row :gutter="20">
        ...
        <el-col :span="6"
                prop="nameZh">
          <iFormItem :label="language('供应商中文名')"
                     required>
        ...
        <el-col :span="6">
          <iFormItem :label="language('供应商简称(中)')"
                     required>
            <iInput v-model="detail.shortNameZh"
                    disabled />
          </iFormItem>
        </el-col>
        <el-col :span="6">
          <iFormItem :label="language('供应商英文名')"
                     required>
            <iInput v-model="detail.nameEn"
                    disabled />
          </iFormItem>
        </el-col>
        <el-col :span="6">
          <iFormItem :label="language('供应商简称(英)')"
                     required>
            <iInput v-model="detail.shortNameEn"
                    disabled />
          </iFormItem>
        </el-col>
        <!-- 2 -->
        <el-col :span="6">
          <iFormItem :label="language('临时号')">
            <iInput v-model="detail.svwTempCode"
                    disabled />
          </iFormItem>
        </el-col>
        <el-col :span="6">
          <iFormItem :label="language('SVW号')">
            <iInput v-model="detail.svwCode"
                    disabled />
          </iFormItem>
        </el-col>
        <el-col :span="6">
          <iFormItem :label="language('SAP号')">
            <iInput v-model="detail.sapCode"
                    disabled />
          </iFormItem>
        </el-col>
        <el-col :span="6">
          <iFormItem :label="language('VW号')">
            <iInput v-model="detail.vmCode"
                    :disabled="!editable" />
          </iFormItem>
        </el-col>
        <!-- 3 -->
        <el-col :span="6">
          <iFormItem :label="language('统一社会信用代码')"
                     required>
            <iInput v-model="detail.socialcreditNo"
                    disabled />
          </iFormItem>
        </el-col>
        <el-col :span="6">
          <iFormItem :label="language('DUNS')">
            <iInput v-model="detail.duns"
                    disabled />
          </iFormItem>
        </el-col>
        <el-col :span="6">
          <iFormItem :label="language('供应商类型')"
                     prop="supplierType">
            <iSelect v-model="detail.supplierType"
                     disabled>
        ...
        <el-col :span="6">
          <iFormItem :label="language('供应商状态')">
            <iInput :value="detail.isActive === '1' ? '正常' : '受控'"
                    disabled />
          </iFormItem>
        </el-col>
        <!-- 4 -->
        <el-col :span="6">
          <iFormItem :label="language('采购员邮箱')"
                     required>
            <iInput v-model="detail.purchaserEmail"
                    disabled />
          </iFormItem>
        </el-col>
        <el-col :span="6">
          <iFormItem :label="language('采购员名称')">
            <iInput v-model="detail.purchaserNameZh"
                    disabled />
          </iFormItem>
        </el-col>
        <el-col :span="6">
          <iFormItem :label="language('采购员科室')">
            <iInput v-model="detail.purchaserDeptNameZh"
                    disabled />
          </iFormItem>
        </el-col>
        <el-col :span="6">
          <iFormItem :label="language('采购员工号')">
            <iInput v-model="detail.purchaserUserNum"
                    disabled />
          </iFormItem>
        </el-col>
        <!-- 5 -->
        <el-col :span="6">
          <iFormItem :label="language('注册日期')"
                     prop="registeredDate">
            <iDatePicker v-model="detail.registeredDate"
                         :disabled="!editable"
                         style="width: 100%" />
          </iFormItem>
        </el-col>
        <el-col :span="6">
          <iFormItem :label="language('注册资本')"
                     prop="registeredCapital">
            <iInput v-model="detail.registeredCapital"
                    :disabled="!editable" />
          </iFormItem>
        </el-col>
        <el-col :span="6">
          <iFormItem :label="language('公司网址')">
            <iInput v-model="detail.companyWebPage"
                    :disabled="!editable" />
          </iFormItem>
        </el-col>
        <el-col :span="6">
          <iFormItem :label="language('是否国外厂商')">
            ...
            <iSelect v-model="detail.isForeignManufacture"
                     :disabled="!editable">
              <el-option v-for="item in dicts.TURE_FALSE"
                         :key="item.code"
                         :value="item.code"
                         :label="item.name" />
            </iSelect>
        ...
        <el-col :span="6">
          <iFormItem :label="language('地址')"
                     prop="companyAddress">
            <iInput v-model="detail.companyAddress"
                    :disabled="!editable" />
          </iFormItem>
        </el-col>
        <el-col :span="6">
          <iFormItem :label="language('是否上市')"
                     prop="isListing">
            <iSelect v-model="detail.isListing"
                     :disabled="!editable">
              <el-option v-for="item in dicts.TURE_FALSE"
                         :key="item.code"
                         :value="item.code"
        ...
        <el-col :span="6">
          <iFormItem :label="language('上市地点')">
            <iSelect v-model="detail.listingAddress"
                     :disabled="!editable">
              <el-option v-for="item in dicts.MARKADDRESS"
                         :key="item.code"
                         :value="item.code"
                         :label="item.name" />
            </iSelect>
        ...
        <el-col :span="6">
          <iFormItem :label="language('股票代码')">
            <iInput v-model="detail.stockCode"
                    :disabled="!editable" />
          </iFormItem>
        </el-col>
        <el-col :span="6">
          <iFormItem :label="language('法定代表人')">
            <iInput v-model="detail.legalPerson"
                    :disabled="!editable" />
          </iFormItem>
        </el-col>
        <!-- <el-col :span="6">
        ...
        <el-col :span="6">
          <iFormItem :label="language('QIANDINGCAIGOUTIAOKUAN', '签订采购条款')">
            ...
            <iInput disabled
                    v-model="detail.purchaseTerm"></iInput>
          </iFormItem>
        </el-col>

        <el-col :span="6">
          <iFormItem :label="language('企业类型')"
                     prop="enterpriseType">
            <iSelect v-model="detail.enterpriseType"
                     :disabled="!editable">
              <el-option v-for="item in dicts.LEGALSTATUS"
                         :key="item.code"
                         :value="item.code"
                         :label="item.name" />
            </iSelect>
        ...
        <!-- 8 -->
        <el-col :span="6">
          <iFormItem :label="language('企业性质(大类)')"
                     prop="epNatureCategory">
            <iSelect v-model="detail.epNatureCategory"
                     :disabled="!editable">
              <el-option v-for="item in dicts.ENTERPRISE_PROPERTY"
                         :key="item.code"
                         :value="item.code"
                         :label="item.name" />
            </iSelect>
        ...
        <el-col :span="6">
          <iFormItem :label="language('企业性质(小类)')"
                     prop="epNatureSubcategory">
            <iSelect v-model="detail.epNatureSubcategory"
                     :disabled="!editable">
              <el-option v-for="item in smallEnterpriseProperty"
                         :key="item.code"
                         :value="item.code"
                         :label="item.name" />
            </iSelect>
        ...
        <el-col :span="6"
                v-if="['GP', 'PD'].includes(supplierType)">
          <iFormItem :label="language('供应商等级')">
            <iInput v-model="detail.supplierGrade"
                    :disabled="!editable" />
          </iFormItem>
        </el-col>
      </el-row>
    ...
    <theDetailBaseTabs :assCompanyVos="detail.assCompanyVos"
