<template>
  ...
  >
    ...
    >
      ...
      <iFormItem :label="$t('供应商中文名')" prop="opcsCompanyNameZh">
        <iInput
      ...
      <iFormItem :label="$t('联系电话')" prop="telephoneM">
        <iInput
      ...
      <iFormItem :label="$t('是否超期')" prop="isExpired">
    ...
    <div slot="footer" v-if="editSta">
      ...
      <iButton @click="dialogVisible = false">{{ $t('退出') }}</iButton>
    </div>
    <div slot="footer" v-else>
      <iButton @click="editSta = true">{{ $t('编辑') }}</iButton>
    </div>
  </iDialog>
</template>
