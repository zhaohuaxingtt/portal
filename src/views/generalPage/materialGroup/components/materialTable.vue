  ...
  <i-card>
    <div class="margin-bottom20 clearFloat">
      <div class="floatright">
        <!-- <el-dropdown
          v-permission="SUPPLIER_MATERIALGROUP_LIST_CONTORL"
          split-button
          trigger="click"
          size="small"
          type="primary"
        ...
        >
          ...
          <el-dropdown-menu slot="dropdown">
            <el-dropdown-item
              v-for="(item, index) in controlListData"
              :key="index"
              :command="item.controlledState"
              >{{ item.name }}</el-dropdown-item
            >
          </el-dropdown-menu>
        </el-dropdown>
        <i-button
          v-permission="SUPPLIER_MATERIALGROUP_LIST_EXPORT"
          @click="handleSave"
        ...
        <i-button
          v-permission="SUPPLIER_MATERIALGROUP_LIST_BDL"
          @click="toApplicationBDL"
          >{{ $t('SUPPLIER_SHENQINGBDL') }}</i-button
        >
        <i-button
          v-permission="SUPPLIER_MATERIALGROUP_LIST_CANCLE"
          @click="getUncontrol"
        ...
        <i-button
          ...
          >{{ language('YIICHUBDL', '移除BDL') }}</i-button
        >

        <i-button
          v-permission="SUPPLIER_MATERIALGROUP_LIST_EXPORTALL"
          @click="exportsAllTable"
          >{{ $t('SUPPLIER_DAOCHUQUANBU') }}</i-button
        >
    ...
    >
      <template #isEffect="scope">
        <span>{{
          scope.row.isEffect
            ? language('SHENPITONGGUO', '审批通过')
            : language('SHENPIZHONG', '审批中')
        }}</span>
      </template>
    ...
    >
      ...
      >
        <el-row :gutter="20" align="middle">
          <el-col :span="10">
            ...
            >
              <iSelect
                v-permission="SUPPLIER_APPLYBDL_VW_LINIE_DEPT"
                @change="handleUser"
                :disabled="isAcc"
          ...
          <el-col :span="10">
            ...
            >
              <iSelect
                v-permission="SUPPLIER_APPLYBDL_VW_LINIE_SOURCER"
      ...
      <div slot="footer" class="flex-between-center">
...
import {
  addControl,
  getPageMaterialGroup,
  updateUncontrol,
  updateAssociated,
  updateControl,
  mbdlCancelAssociated,
  checkCategory
} from '../../../../api/supplier360/material'
  ...
  methods: {
    ...
    async getUncontrol() {
      if (this.selectTableData.length === 1) {
        const res = await updateUncontrol({
          controlledId: this.selectTableData[0].controlledId
        })
        this.resultMessage(
          res,
          () => {
