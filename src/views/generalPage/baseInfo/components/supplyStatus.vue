 ...
 * @LastEditTime: 2021-07-20 19:58:06
 * @LastEditors: 舒杰
	...
	<iCard tabCard collapse :title="$t('GONGHUOZHUANGTAI')" class="margin-top20">
		<iFormGroup row="3">
			...
			<iFormItem v-permission="SUPPLIER_BASEINFO_SUPPLIERSTATUS_ISFACTORY">
				<iLabel :label="$t('SHIFOUYELEIGONGCHANG')" slot="label"></iLabel>
				<iSelect v-model="supplierData.ppSupplierDTO.isFactory">
					<el-option :value="item.code=='1'?1:0" :label="item.name" v-for="(item, index) in fromGroup.TURE_FALSE" :key="index"></el-option>
				</iSelect>
			</iFormItem>
			<!-- 是否隶属华域 -->
			<iFormItem  v-permission="SUPPLIER_BASEINFO_SUPPLIERSTATUS_ISSUBJECTIONCHINA">
				<iLabel :label="$t('SHIFOULISHUHUOYU')" slot="label"></iLabel>
			...
			<iFormItem v-permission="SUPPLIER_BASEINFO_SUPPLIERSTATUS_SUPPLYPROPERTIES">
				<iLabel :label="$t('GONGHUOLEIXING')" slot="label"></iLabel>
			...
			<iFormItem v-permission="SUPPLIER_BASEINFO_SUPPLIERSTATUS_SUPPLYTYPE">
				<iLabel :label="$t('GONGHUOLINGJIANLEIXING')" slot="label"></iLabel>
			...
			<!-- 定点行协议 -->
			<iFormItem>
				<iLabel :label="$t('SUPPLIER_DDXXX')" slot="label"></iLabel>
				<iSelect v-model="supplierData.ppSupplierDTO.isSign">
					<el-option :value="item.code" :label="item.code==='1'?'已签署':'未签署'" v-for="(item, index) in fromGroup.TURE_FALSE" :key="index"></el-option>
				</iSelect>
			</iFormItem>
		</iFormGroup>
	</iCard>
