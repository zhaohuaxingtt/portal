...
<template>
  <div class="home">
    <iCard tabCard collapse :title="$t('LK_JICHUXINXI')">
      <template slot="header-control">
        <iButton @click="onJump360">{{ $t('FANHUIGONGYINSHANG360') }}</iButton>
      </template>
      <iFormGroup row="3">
        <iFormItem v-for="(item,index) in baseInfoTitle" :key="index" :v-permission="item.permission">
          <iLabel :label="$t(item.name) " :required="item.required" :icons="item.icons"
                  :tip="$t(item.tip)" slot="label"></iLabel>
          <iText v-if="item.key=='svwTempCode' || item.key=='svwCode'">{{baseMsg[infoVo][item.key]}}</iText>
          <iInput v-else-if="item.key=='vmCode' && !item.disabled" v-model="baseMsg[infoVo][item.key]"></iInput>
