...
;(function (factory) {
  ...
  $.fn.orgchart = function (options) {
    ...
    switch (options) {
      case 'buildHierarchy':
        return buildHierarchy.apply(
          this,
          Array.prototype.splice.call(arguments, 1)
        )
      case 'addChildren':
        return addChildren.apply(
          this,
          Array.prototype.splice.call(arguments, 1)
        )
      case 'addParent':
      ...
      case 'addSiblings':
        return addSiblings.apply(
          this,
          Array.prototype.splice.call(arguments, 1)
        )
      case 'removeNodes':
        return removeNodes.apply(
          this,
          Array.prototype.splice.call(arguments, 1)
        )
      case 'getHierarchy':
        return getHierarchy.apply(
          this,
          Array.prototype.splice.call(arguments, 1)
        )
      default:
    ...
    if ($.type(data) === 'object') {
      if (data instanceof $) {
        ...
        buildHierarchy(
          $chart,
          opts.ajaxURL ? data : attachRel(data, '00'),
          0,
          opts
        )
      }
  ...
  function hideDescendants($node) {
    ...
    var $lines = $visibleNodes
      .closest('table')
      .closest('tr')
      .prevAll('.lines')
      .css('visibility', 'hidden')
    $visibleNodes
      .addClass('slide slide-up')
      .eq(0)
      .one('transitionend', function () {
  ...
  function startLoading($arrow, $node, options) {
    ...
    $(
      '.oc-export-btn' +
        (options.chartClass !== '' ? '.' + options.chartClass : '')
    ).prop('disabled', true)
    return true
  }
  ...
  function switchVerticalArrow($arrow) {
    $arrow.toggleClass('el-icon-arrow-up').toggleClass(' el-icon-arrow-down')
  }
  ...
  function switchHorizontalArrow($node) {
    ...
    if (
      ...
      $node
        .children('.leftEdge')
        .toggleClass('el-icon-arrow-right', sibsVisible)
        .toggleClass('el-icon-arrow-left', !sibsVisible)
      $node
        .children('.rightEdge')
        .toggleClass('el-icon-arrow-left', sibsVisible)
        .toggleClass('el-icon-arrow-right', !sibsVisible)
  ...
  function createNode(nodeData, level, opts) {
    ...
    var $nodeDiv = $(
      '<div' +
        ...
        (nodeData[opts.nodeId]
          ? ' id="org_node_id_' + nodeData[opts.nodeId] + '"'
          : '') +
        '>'
    )
      .addClass(
    ...
    const isRoot = nodeData.parentId === 0 || nodeData.parentId === null
    if (Number(flags.substr(0, 1)) && !isRoot) {
    ...
    $nodeDiv.on('mouseenter mouseleave', function (event) {
      ...
      if (event.type === 'mouseenter') {
        if ($topEdge.length) {
          ...
          $topEdge
            .toggleClass('el-icon-arrow-up', !flag)
            .toggleClass(' el-icon-arrow-down', flag)
        }
        ...
        $node
          .children('.edge')
          .removeClass(
            'el-icon-arrow-up  el-icon-arrow-down el-icon-arrow-right el-icon-arrow-left'
          )
      }
    })
    ...
    $nodeDiv.on('click', function (event) {
      ...
      if (opts.handleNodeClick) {
        opts.handleNodeClick(nodeData)
      }
    })
      ...
      $nodeDiv
        ...
        .on('dragover', function (event) {
          ...
          var $dragZone = $dragged
            ...
            .find('.node:first')
          if (
            $dragged.closest('table').find('.node').index($dropZone) > -1 ||
            (opts.dropCriteria &&
              !opts.dropCriteria($dragged, $dragZone, $dropZone))
          ) {
        ...
        .on('drop', function (event) {
          ...
          if (!$dropZone.closest('tr').siblings().length) {
            ...
            $dropZone
              ...
              .parent()
              .after(
                '<tr class="lines child-1"><td colspan="2"><div class="down"></div></td></tr>' +
                  '<tr class="lines child-2"><td class="right">&nbsp;</td><td class="left">&nbsp;</td></tr>' +
                  '<tr class="nodes"></tr>'
              )
              .siblings(':last')
              .append(
  ...
  function buildHierarchy($appendTo, nodeData, level, opts, callback) {
    ...
    if (hasChildren) {
      ...
      var linesRow =
        ...
        isHidden +
        ' child-' +
        ($childNodes.length === 1 ? 2 : 2 + $childNodes.length) +
        '"><td class="right">&nbsp;</td>'
  ...
  function buildParentNode($currentRoot, nodeData, opts, callback) {
    ...
    $.when(
      createNode(
        nodeData,
        0,
        opts || $currentRoot.closest('.orgchart').data('options')
      )
    )
      .done(function ($nodeDiv) {
        $table.append(
          $nodeDiv
            .removeClass('slide-up')
            .addClass('slide-down')
            .wrap('<tr class="hidden"><td colspan="2"></td></tr>')
            .closest('tr')
        )
        $table.append(
        ...
        var linesRow =
          '<td class="right">&nbsp;</td><td class="left">&nbsp;</td>'
  ...
  function buildSiblingNode($nodeChart, nodeData, opts, callback) {
    ...
    var newSiblingCount = nodeData.siblings
            ...
            if (existingSibligCount > 1) {
              complementLine(
                $siblingTds
                  .eq(0)
                  .before(
                    $nodeChart.closest('td').siblings().andSelf().unwrap()
                  ),
                siblingCount,
                existingSibligCount
              )
