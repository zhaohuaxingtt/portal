  ...
  >
    ...
    >
      <iFormItem :label="language('原材料大类')" prop="generalCategoryCode">
        <iSelect
          v-model="formContent.generalCategoryCode"
          :placeholder="language('请选择')"
          v-if="!editBtnSta"
          @change="selectedCode"
          :disabled="this.papgeTitle != '新增'"
        >
          <el-option
      ...
      <iFormItem :label="language('中类描述(中)')" prop="rawMaterialDesc">
        <div v-if="this.papgeTitle == '新增'">
          <el-autocomplete
            v-model="formContent.rawMaterialDesc"
            filterable
      ...
      <iFormItem :label="language('材料中类')" prop="rawMaterialCode">
        ...
        >
          ...
          <iInput
            v-model="rawMaterialCode.seconde"
            :disabled="RawCodeSe"
            slot="append"
            maxLength="3"
            @input="limitNumber"
            @blur="toChangemtzCode"
          ></iInput>
        </p>
      ...
      <iFormItem :label="language('中类描述(英)')" prop="rawMaterialNameEn">
        <iInput
          v-model="formContent.rawMaterialNameEn"
      ...
      <iFormItem :label="language('中类计量单位')" prop="rawMaterialCountUnit">
        <iSelect
          v-model="formContent.rawMaterialCountUnit"
          v-if="!editBtnSta"
          :placeholder="language('请选择')"
          @change.once="changeUnit"
        >
          <el-option
      ...
      <iFormItem :label="language('原材料牌号')" prop="mtzRawMaterialCode">
        ...
        >
          ...
          <iInput
            v-model="mtzRawMaterialCode.seconde"
            slot="append"
            @input="inputMtzRawCode"
            maxLength="3"
            :disabled="this.papgeTitle != '新增'"
          ></iInput>
        </p>
        <iInput
          v-model="formContent.mtzRawMaterialCode"
          disabled
          v-else
        ></iInput>
      </iFormItem>
      <iFormItem
        :label="language('原材料计量单位')"
        prop="mtzRawMaterialCountUnit"
      >
        <iSelect
          v-model="formContent.mtzRawMaterialCountUnit"
          v-if="!editBtnSta"
          :placeholder="language('请选择')"
        >
          <el-option
...
export default {
  ...
  methods: {
    toChangemtzCode(val) {
      ...
      this.formContent.rawMaterialCode =
        this.rawMaterialCode.first + '' + this.rawMaterialCode.seconde
      this.mtzRawMaterialCode.first = 'M' + this.formContent.rawMaterialCode
      getMaxCode({ rawMaterialCode: this.formContent.rawMaterialCode }).then(
        (val) => {
          if (val.code == 200) {
            if (val.data) {
              const data = val.data.mtzRawMaterialCode
              this.mtzRawMaterialCode.seconde = (
                Number(data.substring(data.length - 3, data.length)) + 1
              )
                .toString()
                .padStart(3, 0)
              this.formContent.mtzRawMaterialCode =
                this.formContent.rawMaterialCode +
                '' +
                this.mtzRawMaterialCode.seconde
            } else {
            ...
            this.formContent.rawMaterialCode =
              this.rawMaterialCode.first + '' + this.rawMaterialCode.seconde
            this.mtzRawMaterialCode.first =
              'M' + this.formContent.rawMaterialCode
            this.formContent.mtzRawMaterialCode =
              this.formContent.rawMaterialCode +
              '' +
              this.mtzRawMaterialCode.seconde
    ...
    inputMtzRawCode(val) {
      ...
      this.formContent.mtzRawMaterialCode =
        this.mtzRawMaterialCode.first + '' + this.mtzRawMaterialCode.seconde
    },
    handleSelect() {
      let code = ''
      if (this.restaurantsOption) {
        this.restaurantsOption.forEach((item) => {
          if (
            item.name ==
            this.formContent.rawMaterialDesc.substring(
              6,
              this.formContent.rawMaterialDesc.length
            )
          ) {
            code = item.code
            this.formContent.rawMaterialNameEn = item.nameEn
            this.rawMaterialCode.seconde = code.substring(
              this.rawMaterialCode.first.length
            )
          }
        })
      }
      ...
      getRawCodeOptions({ generalCategoryCode: val }).then((val) => {
        if (val.code == 200) {
          ...
          if (options) {
            ...
            this.rawMaterialCode.seconde = (
              Number(options.pop().code.substring(2, 5)) + 1
            )
              .toString()
              .padStart(3, 0)
    ...
    save() {
      this.$refs['rulesRef'].validate((valid) => {
        if (valid) {
          ...
          console.log(
            this.formContent.rawMaterialDesc,
            'this.formContent.rawMaterialDesc'
          )
          if (this.restaurantsOption) {
            this.restaurantsOption.forEach((item) => {
    ...
    edit() {
      this.mtzRawMaterialCode.first =
        this.formContent.mtzRawMaterialCode.length > 0
          ? this.formContent.mtzRawMaterialCode.substring(
              0,
              this.formContent.mtzRawMaterialCode.length - 3
            )
          : ''
      this.mtzRawMaterialCode.seconde =
        this.formContent.mtzRawMaterialCode.length > 0
          ...
          : ''
      this.rawMaterialCode.first =
        this.formContent.rawMaterialCode.length > 0
          ? this.formContent.rawMaterialCode.substring(
              0,
              this.formContent.rawMaterialCode.length - 3
            )
          : ''
      this.rawMaterialCode.seconde =
        this.formContent.rawMaterialCode.length > 0
