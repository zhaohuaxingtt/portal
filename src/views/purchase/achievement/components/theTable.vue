    ...
    <iCard>
        <div class="margin-bottom20 clearFloat">
            <div class="floatright">
                <!--新增-->
                <iButton class="ml10" @click="add" v-permission="ACHIEVEMENTMGT_LIST_NEWADD">
                    {{ $t('LK_XINZENG') }}
                </iButton>
                <!--失效-->
                <iButton class="ml10" @click="invalid" v-permission="ACHIEVEMENTMGT_LIST_INVALID">
                    {{ $t('APPROVAL.INVALIDATION') }}
                </iButton>
                <!--产量管理-->
                <iButton class="ml10" @click="openOutputManagemen" v-permission="ACHIEVEMENTMGT_LIST_YIEIDMANAGE">
                    {{ $t('EKL_YJGL_CLGL') }}
                </iButton>
                <!--业绩金额调整-->
                <iButton class="ml10" @click="openAmountAdjustment" v-permission="ACHIEVEMENTMGT_AMOUNT_ADJUSTMENT">
...
<script>
    ...
    import amountAdjustDialog from '../components/amountAdjustDialog';
    import spareTargetDialog from '../components/spareTargetDialog';
    import {
        getAchievementList,
        getYear,
        invalid,
    ...
    export default {
        ...
        methods: {
            ...
            openPage(val) {
                if (val.value == '刷新') {
                    this.$alert('此操作不会立即执行,预计每日可查看执行结果，同时该版本将被覆盖 是否继续刷新?', '提示', {
                    ...
                    if(val.status == 0 || val.status == 1) {
                        return iMessage.error(`${ this.$i18n.locale === 'zh' ? '此状态下无法查看业绩基础明细，请重新创建业绩基础' : 'unable to view the performance base details in the status of import failure. Please re create the performance base'
                            }`)
                    }
                    ...
                    if(val.status == 0 || val.status == 1) {
                        return iMessage.error(`${ this.$i18n.locale === 'zh' ? '此状态下无法查看业绩追踪明细，请重新创建业绩追踪' : 'unable to view the performance base details in the status of import failure. Please re create the performance base'
                            }`)
                    }
                    let {id, year, version, title, status, billType, type} = val
                    version = val.modelVersion
            ...
            invalid() {
                ...
                if (id) {
                    if (status == 3 || status == 11) {
                        this.$confirm('已有用户确认过的业绩基础明细，您是否确认要对该业绩基础表做失效处理?', '提示', {
                            confirmButtonText: '确定',
                            cancelButtonText: '取消',
                            type: 'warning'
                        }).then(() => {
