        ...
        <div class="margin-bottom20 clearFloat">
            <div class="floatright">
                <div v-if="!editStatus">
                    ...
                    <!--转派-->
                    <iButton @click="taskTransfer"
                             v-if="isAuth(whiteBtnList,'ACHIEVEMENT_DETAIL_PAGE_REASSIGNMENT')&&!isForward">
                        {{ $t('APPROVAL.TASK_TRANSFER') }}
                    </iButton>
                    <!--确认-->
                    <iButton @click="confirm" v-if="isAuth(whiteBtnList,'ACHIEVEMENT_DETAIL_PAGE_CONFIRM')&&!isForward">
                        {{ $t('rfq.RFQINQUIRE') }}
                    </iButton>
                    <!--导出-->
                    <iButton @click="download" v-if="isAuth(whiteBtnList,'ACHIEVEMENT_DETAIL_PAGE_EXPORT')">
                        {{ $t('APPROVAL.EXPORT')}}
                    </iButton>
                </div>
                <div v-else>
                    ...
                    <!--保存-->
                    <iButton @click="save">{{ $t('APPROVAL.SAVE') }}</iButton>
                </div>
  ...
  export default {
    ...
    data() {
      return {
        ...
        partType: [{code: 'P', name: 'P'}, {code: 'L', name: 'L'}, {code: 'S', name: 'S'}, {code: 'A', name: 'A'}],
    ...
    methods: {
      ...
      setPartType(val) {
        updatepartType({id: val.id, type: val.partType}).then(res => {
          if (res.result) {
            if (res.data) {
              this.tableListData[val.index].confirmPrice = toThousands(res.data.price)
      ...
      getSupplierList() {
        getSupplier({supplierName: ''}).then(res => {
          if (res.result) {
            ...
            this.tempSupplierSelectList = this.tempSupplierSelectList.filter(item => {
              return item.supplierId && item.nameZh && item.sapCode
            })
            this.tempSupplierSelectList = unique(this.tempSupplierSelectList, 'sapCode')
            ...
            this.$nextTick(() => {
              this.$set(this.selectPropsOptionsObject, 'supplierName', this.supplierSelectList)
            })
          }
        })
      },
        ...
        try {
          ...
          if (res.result) {
            ...
            this.tableListData.map((item) => {
              ...
              item.priceConfirm = item.priceConfirm?toThousands(item.priceConfirm):''
              item.procAmount = item.procAmount ? toThousands(item.procAmount) : ''
              item.carconfigSixcodeVolumn = item.carconfigSixcodeVolumn ? toThousands(item.carconfigSixcodeVolumn) : ''
              item.janCarvolumn = item.janCarvolumn ? toThousands(item.janCarvolumn) : ''
              item.febCarvolumn = item.febCarvolumn ? toThousands(item.febCarvolumn) : ''
              item.marCarvolumn = item.marCarvolumn ? toThousands(item.marCarvolumn) : ''
              item.aprCarvolumn = item.aprCarvolumn ? toThousands(item.aprCarvolumn) : ''
              item.mayCarvolumn = item.mayCarvolumn ? toThousands(item.mayCarvolumn) : ''
              item.junCarvolumn = item.junCarvolumn ? toThousands(item.junCarvolumn) : ''
              item.julCarvolumn = item.julCarvolumn ? toThousands(item.julCarvolumn) : ''
              item.augCarvolumn = item.augCarvolumn ? toThousands(item.augCarvolumn) : ''
              item.sepCarvolumn = item.sepCarvolumn ? toThousands(item.sepCarvolumn) : ''
              item.octCarvolumn = item.octCarvolumn ? toThousands(item.octCarvolumn) : ''
              item.novCarvolumn = item.novCarvolumn ? toThousands(item.novCarvolumn) : ''
              item.decCarvolumn = item.decCarvolumn ? toThousands(item.decCarvolumn) : ''
      ...
      bulkEdit() {
        if (this.selectTableData && this.selectTableData.length == 1) {
          ...
          return iMessage.error(`${ this.$i18n.locale === 'zh' ? '请选择一条数据' : 'please select row data' }`)
        }
      },
      // 批量编辑回调
      bulkEditCB(val) {
      ...
      save() {
        ...
        let state = list.some(item => {
          ...
          let str = item.priceConfirm.substring(item.priceConfirm.indexOf('.') + 1)
        ...
        list.forEach(item => {
          let obj = {
            ...
            supplierNameZh: item.inputVal ? item.supplierNameZh : item._supplierNameZh,
          ...
          if (item.changeType) {
            obj.confirmPrice = item.confirmPrice ? delcommafy(item.confirmPrice) : item.confirmPrice1,// 参考价格
        ...
        if (data.length) {
          this.tableLoading = true
          updateSeriesbaseDetail(data).then(res => {
            if (res.result) {
              this.tableLoading = false
              this.getTableList()
...
//                        iMessage.error(this.$t('APPROVAL.OPERATION_FAILED'))
      ...
      confirm() {
        ...
        if (this.selectTableData && this.selectTableData.length >= 1) {
          ...
          this.tableLoading = true
          affirmSeriesbaseDetail(obj).then(res => {
            if (res.result) {
              this.tableLoading = false
...
//                this.formData.pageSize = 1
        this.tableLoading = true
        exportSeriesbaseDetail(this.formData).then(res => {
          this.tableLoading = false
        }).catch((err) => {
          this.tableLoading = false
//                    iMessage.error(this.$t('APPROVAL.OPERATION_FAILED'))
