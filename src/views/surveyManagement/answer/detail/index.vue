<template>
  <iPage v-loading="loading">
    ...
    <iCard>
      <component v-if="component" :is="component" :name="name"  v-model="previewData" @subClick="handlerSubClick"></component>
    </iCard>
  </iPage>
</template>

<script>
import { iPage, iInput, iButton,iCard } from "rise";
// import { queryForAnswer } from "@/api/survey/answer.js";
import { findById, saveAnswer, queryForAnswer } from "@/api/survey/survey.js";
...
export default {
  ...
  max() {
    return function (da) {
      console.log(671, da);
      const multipleRule = da?.multipleRule ? JSON.parse(da?.multipleRule) : {};
  ...
  methods: {
    handlerSubClick(previewData,sonQuestions){
      ...
      //       }
      // })
      if(e.type != 9 && (!e.answer || e.answer.length == 0) && e.requiredQuestion) {
          flag = true;
        }
      ...
      if (document.getElementsByClassName('el-message').length === 0) {
          ...
          console.log('错误信息进来了',document.getElementsByClassName('el-message'))
          return
        }
    ...
    doNotMake(questions) {
      ...
      questions.map(e => {
        if((!e.answer || e.answer.length == 0 || e.answer == null ) && e.requiredQuestion == true) {
          // if(e.type == 3 && e.answer.trim() =="") flag = true;
          flag = true;  
        }
      });
      ...
      if(data.type == 1) {
        ...
        obj = {
         ...
         // answer: data.answer ? data.answer[0].attachmentId + '.' + data.answer[0].upLoadFileName : null
          answer: data.answer ? (data.answer[0].attachmentId) + '.' + (data.answer[0].upLoadFileName.length > 55 ? data.answer[0].upLoadFileName.slice(-50) : data.answer[0].upLoadFileName) : null
        }
      }else if(data.type == 9) {
        obj = {
          questionId: data.id,
    ...
    query(e) {
      if(this.state == 5){
        ...
        queryForAnswer(e).then((res) => {
          ...
          this.previewData.questions = this.previewData.questions.map(item  => {
