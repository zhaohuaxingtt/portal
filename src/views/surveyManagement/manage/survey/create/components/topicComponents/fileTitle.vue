<template>
  ...
  >
    ...
    <div style="margin-top: 1.25rem; margin-left: 1rem" v-show="editor">
      ...
      <el-row class="form-row">
        <div class="form-item-row1-col3">
          <iFormItem label="上传格式设置" prop="file">
            <iLabel
              :label="$t('上传格式设置')"
              slot="label"
            ...
            <div class="uploadSet" v-if="isParap == fasle">
              <div class="title">限制上传文件类型</div>
              <div v-for="item in uploadOptions" :key="item.name">
                <el-form-item :label="item.name + '：'" class="fileItem">
                  <el-checkbox
            ...
            <div class="uploadSetP" v-else>
              <div class="titleP">限制上传文件类型</div>
              <div v-for="item in uploadOptions" :key="item.name">
                <el-form-item :label="item.name + '：'" class="fileItemP">
                  <el-checkbox
                    :indeterminate="item.isIndeterminate"
                    v-model="item.selected"
                    class="itemP"
