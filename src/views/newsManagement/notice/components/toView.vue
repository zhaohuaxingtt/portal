<template>
  <iPage>
    <div v-show="card">
      <div class="page">
        <div class="addNotice">{{language('NEWS_CHAKANGONGGAO','查看公告')}}</div>
        <div>
          <iButton
            @click="handleSave(0)"
            :disabled="
              this.noticeStatus == 1 || this.noticeStatus == 2 || saveDisabled
            "
            >{{ language('NEWS_BAOCUN', '保存') }}</iButton
          >
          <iButton
            @click="handleReset"
            :disabled="this.noticeStatus == 1 || this.noticeStatus == 2"
            >{{ language('LK_ZHONGZHI', '重置') }}</iButton
          >
          <iButton @click="handlePreview">{{ language('NEWS_YULAN', '预览')  }}</iButton>
          <iButton
            @click="handleSave(1)"
            :disabled="this.noticeStatus == 1 || this.noticeStatus == 2"
            >{{ language('NEWS_FABU', '发布') }}</iButton
          >
      ...
      <iCard>
        ...
        <div class="card-title top-buttom">正文</div>
        <div class="form-row">
        ...
        <div class="form-row">
          <div>
            <el-upload
              ...
              multiple
              :disabled="uploadLoading || ruleForm.status!==0"
            >
              <div class="upload-text">
                <iButton :disabled="uploadLoading || ruleForm.status!==0" :loading="uploadLoading">{{language('NEWS_TIANJIAFUJIAN', '添加附件')}}</iButton>
...
export default {
    ...
    return {
      ...
      rules: {
        title: [
          ...
          { min: 1, max: 80, message: "最大输入长度80", trigger: "blur" },
        ],
        type: [
          { required: true, message: this.language('NEWS_BITIAN',"必填"), trigger: ["blur", "change"] },
        ],
  ...
  methods: {
    ...
    handleClose() {
      ...
      if (
        this.ruleForm.userGroup?.length === 0 ||
        this.ruleForm.userGroup === null
      ) {
        this.ruleForm.publishRange = "";
      }
