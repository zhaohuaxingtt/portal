      ...
      this.$refs["formComponent"].$refs["ruleForm"].validate((valid) => {
        if (valid) {
          ...
          if (formData.picUrl === null || formData.picUrl === "") {
            ...
            this.$message.error(this.language('NEWS_TTLJHZWNRBNTSWK',"头条链接和正文内容不能同时为空！"));
            return;
          } else if (
            formData.category > 0 &&
            formData.type === 0 &&
            ...
            this.$message.error(this.language('NEWS_ZWNRBNWK',"正文内容不能为空！"));
            return;
          } else if (formData.type === 1 && formData.newsAtlas.length == 0) {
            this.$message.error(this.language('NEWS_TJZSSCYZ',"图集至少上传一张！"));
            return;
          } else if (
            formData.type === 2 &&
          ...
          if (
            ...
            this.$confirm(this.language('NEWS_SFQDFBCXW',"是否确定发布此新闻 ？"), this.language('NEWS_TISHI',"提示"), {
