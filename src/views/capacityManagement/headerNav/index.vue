...
export default {
  ...
  created() {
    let pathList = this.$route.matched.map((item) => item.path)
    this.group = TAB.find((i) => pathList.includes(i.url))?.group ?? ''
  ...
  methods: {
    ...
    judgeMenuPermission() {
      ...
      if (mentItemOfRightNav) {
        ...
        // 没有菜单权限
        if (!this.whiteBtnList[permissionKey]) {
          const hasPermissionMenu = this.getHasPermissionMenu(
            this.heaederSubMenu
          )
          if (hasPermissionMenu) {
            return this.$router.push({
            ...
            const otherPermissionMenu = this.getHasPermissionMenu(
              this.navListLeft
            )
