  ...
  <iCard tabCard collapse :title="$t('GONGSIGAIKANG')">
    ...
    >
      ...
      <iFormItem prop="supplierDTO.legalPerson">
        ...
        <iInput
          v-model="supplierData.supplierDTO.legalPerson"
        ...
        <iInput
          v-model="supplierData.supplierDTO.legalPerson"
          v-else
          v-permission.edit="SUPPLIER_BASEINFO_COMPANY_LEGALPERSON_GP"
          :placeholder="$t('LK_QINGSHURU') + $t('LegalPerson')"
        ></iInput>
      </iFormItem>
      <!-- 企业规模 -->
      <iFormItem
        prop="supplierDTO.companySize"
        v-permission="SUPPLIER_BASEINFO_COMPANY_COMPANYSIZE_GP"
      >
      ...
      <!-- 经营状态 -->
      <iFormItem
        prop="supplierDTO.businessState"
        v-permission="SUPPLIER_BASEINFO_COMPANY_BUSINESSSTATUS_GP"
      >
      ...
      <!-- 企业类型 -->
      <iFormItem
        prop="supplierDTO.enterpriseType"
        v-permission="SUPPLIER_BASEINFO_COMPANY_ENTERPRISETYPE_GP"
      >
      ...
      <!-- 企业性质（大类） -->
      <iFormItem
        prop="supplierDTO.epNatureCategory"
        v-permission="SUPPLIER_BASEINFO_COMPANY_EPNATURECATEGORY_GP"
      >
        <iLabel :label="$t('epNatureCategory')" required slot="label"></iLabel>
        <iSelect
          v-model="supplierData.supplierDTO.epNatureCategory"
      ...
      <!-- 企业性质（小类） -->
      <iFormItem
        prop="supplierDTO.epNatureSubcategory"
        v-permission="SUPPLIER_BASEINFO_COMPANY_COMPANYNATURE_GP"
      >
      ...
      <!-- 所属行业 -->
      <iFormItem prop="" v-permission="SUPPLIER_BASEINFO_COMPANY_INDUSTRY_GP">
        <iLabel :label="$t('Industry')" required slot="label"></iLabel>
        <iSelect
          v-model="supplierData.supplierDTO.industry"
      ...
      <!-- 行业代码 -->
      <iFormItem v-permission="SUPPLIER_BASEINFO_COMPANY_INDUSTRYCODE_GP">
        <iLabel :label="$t('industryCode')" required slot="label"></iLabel>
        <iText>{{ isIndustryCode }}</iText>
      </iFormItem>
      <!-- 业务范围 -->
      <iFormItem
        prop="supplierDTO.businessOfScope"
        v-permission="SUPPLIER_BASEINFO_COMPANY_SCOPEOFBUSINESS_GP"
      >
      ...
      <!-- 是否上市 -->
      <iFormItem
        prop="supplierDTO.isListing"
        v-permission="SUPPLIER_BASEINFO_COMPANY_ISLISTING_GP"
      >
        <iLabel :label="$t('IsListing')" required slot="label"></iLabel>
        <iSelect
          v-model="supplierData.supplierDTO.isListing"
      ...
      <!-- 上市地点 -->
      <iFormItem
        prop="supplierDTO.listingAddress"
        :rules="listingAddress"
        ref="listingAddressItem"
        v-permission="SUPPLIER_BASEINFO_COMPANY_LISTINGADDRESS_GP"
      >
        <iLabel
          :label="$t('listingAddress')"
          :required="isListing"
          slot="label"
        ></iLabel>
        <iSelect
          v-model="supplierData.supplierDTO.listingAddress"
          :disabled="!isListing"
        >
          <el-option
            :value="item.code"
      ...
      <!-- 股票代码 -->
      <iFormItem
        prop="supplierDTO.stockCode"
        :rules="stockCode"
        ref="stockCodeItem"
        v-permission="SUPPLIER_BASEINFO_COMPANY_CODEOFSTOCK_GP"
      >
        <iLabel
          :label="$t('GUPIAODAIMA')"
          :required="isListing"
        ...
        ></iLabel>
        <iInput
          v-model="supplierData.supplierDTO.stockCode"
      ...
      <!-- 注册资本 -->
      <iFormItem
        prop="supplierDTO.registeredCapital"
        v-permission="SUPPLIER_BASEINFO_COMPANY_REGISTEREDCAPITAL_GP"
      >
        <iLabel
          :label="$t('SUPPLIER_ZHUCEZIBEN')"
          required
          slot="label"
        ></iLabel>
        <iInput
          v-model="supplierData.supplierDTO.registeredCapital"
          :placeholder="$t('LK_QINGSHURU') + $t('SUPPLIER_ZHUCEZIBEN')"
        ></iInput>
      </iFormItem>
      <!-- 注册日期 -->
      <iFormItem
        prop="supplierDTO.registeredDate"
        v-permission="SUPPLIER_BASEINFO_COMPANY_REGISTEREDDATE_GP"
      >
      ...
      <!-- 供货属性 -->
      <iFormItem
        prop="supplierDTO.supplyProperty"
        v-permission="SUPPLIER_BASEINFO_COMPANY_SUPPLIERSTATUS_GP"
      >
      ...
      <!-- 财务状况 -->
      <iFormItem v-permission="SUPPLIER_BASEINFO_COMPANY_CAIWUZHAUNGKUANG_GP">
      ...
      <!-- 财务趋势 -->
      <iFormItem v-permission="SUPPLIER_BASEINFO_COMPANY_FINANCIALTREND_GP">
      ...
      <!-- 是否大陆厂商 -->
      <iFormItem v-permission="SUPPLIER_BASEINFO_COMPANY_ISFOREIGN_GP">
        <iLabel
      ...
      <!-- 是否黑名单 -->
      <iFormItem v-permission="SUPPLIER_BASEINFO_COMPANY_ISBLACK_GP">
        <iLabel
          :label="language('SHIFOUHEIMINGDAN', '是否黑名单')"
          slot="label"
        ></iLabel>
        <iText>{{
      ...
      <iFormItem v-if="$route.query.subSupplierType !== 'GP'">
        ...
        <iText>
          {{
            isPP
              ? supplierData.ppSupplierDTO.isSignedClause
                ? supplierData.ppSupplierDTO.signTime
                : this.language('FOU', '否')
              : supplierData.gpSupplierInfoVO.isSignedClause
              ? supplierData.ppSupplierDTO.signTime
              : this.language('FOU', '否')
          }}</iText
        >
...
import { getCityInfo, isBlack } from '@/api/dictionary'
...
export default {
  ...
  computed: {
    ...
    isIndustryCode() {
      return this.supplierData.supplierDTO.industryCode
        ? pad(this.supplierData.supplierDTO.industryCode, 4)
        : ''
    },
    isPP() {
      ...
      if (this.$route.query.supplierToken)
        isBlack({ supplierToken: this.$route.query.supplierToken }).then(
          (res) => {
            ...
            this.$set(this.supplierData.baseInfo, 'isBlacklist', code)
          }
        )
    },
