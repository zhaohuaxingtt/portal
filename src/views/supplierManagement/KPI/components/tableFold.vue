        ...
        <table class="customTab" cellspacing="0" cellpadding="0">
            <tr class="tbheard">
                <td v-for="(x,index) in tableTittle" :key="index">
                    <div :class="dashedStatus(index)">
                    {{iconStatusinterval(x,index)}}{{x.label}}{{iconStatusinterval2(x,index)}}
...
export default {
    ...
    methods:{
        ...
        iconStatusinterval2(x,index){
            if(!this.isFold){
                let idxstar= this.tableTittle.findIndex(x=>{return x.start==false})
                if(index==idxstar){
                    return '）'
                }
            }else{
