...
<template>
  <div>
    <iCard>
      <div class="flex-between-center-center margin-bottom24">
        ...
        <div class="flex-between-center-center">
          ...
          <el-switch @change="handleLine" v-model="isLine" active-color="#1660F1" inactive-color="#cacccd">
          </el-switch>
        </div>
        <div>
          <iButton v-if="status" @click="handleUnfold">{{language('ZHANKAI','展开')}}</iButton>
      ...
      <div v-if="status">
        ...
        <div class="margin-bottom20 flex-end">
          ...
          <!-- 导出报警信-->
          <iButton :loading="exportAlarmLetterLoading" @click="exportAlarmLetter(selectTableData)">{{ language('DAOCHUBAOJINGXING','导出报警信')}}</iButton>
          <iButton :loading="exportFeedbackLoading" @click="handleExportFeedback(selectTableData)">{{ language('DAOCHUFANKUIBIAO','导出反馈表')}}</iButton>
          <iButton :loading="sendFeedbackLoading" @click="handleSendFeedback(selectTableData)">{{ language('FASONGFANKUIBIAO','发送反馈表')}}</iButton>
        </div>
      </div>
      <el-table height="360" tooltip-effect='light' row-key="number" :tree-props="{children:'partNumList'}" v-loading="tableLoading" :ref="'multipleTable'" :data="tableListData" @selection-change="handleSelectionChange($event,index)">
        <el-table-column align="center" show-overflow-tooltip type="selection" width="55"> </el-table-column>
        <el-table-column type="" align="center" show-overflow-tooltip prop="supplierName" :label="language('GONGYINGSHANGMINGCHEN','供应商名称')" />
        <el-table-column align="left" show-overflow-tooltip prop="partNumSize" :label="language('LINGJIANSHULIANGLINGJIANHAO','零件数量（零件号）')" width="135">
        </el-table-column>
        <el-table-column align="center" show-overflow-tooltip v-if="eventDetail.createType===language('ZIDONGCHUANGJIAN','自动创建')" prop="impactLevel" :label="language('SOUYINGXIANGCHENGDU','受影响程度')">
        </el-table-column>
        <el-table-column align="center" prop="feedbackStatus" :label="language('FANKUIZHUANGTAI1','反馈状态')">
    ...
    <!-- 突发事件 -->
    <emergencyDialog :eventDetail="eventDetail" :tableListData="tableListData" v-model="emergencyDialog" />
    <!-- 导出当页 -->
    <exportDialog v-model="exportDialog" />
    <!--编辑  -->
    <createEmergenciesDialog :eventDetail="eventDetail" v-model="createEmergenciesDialog" />
  </div>
</template>
