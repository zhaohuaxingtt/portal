...
<template>
  <iDialog
    :title="language('TIANJIASHANGYOUGONGYINGSHANG', '添加上游供应商')"
    width="90%"
    :visible.sync="value"
    show-close
    @close="clearDiolog"
  >
    ...
    <el-row type="flex" :gutter="40">
      ...
      <el-col :span="4">
        <iInput
          ...
          :placeholder="
            language('GONGYINGSHANGMINGCHEN_SAPHAO', '供应商名称/SAP号')
          "
        ></iInput>
      </el-col>
      <el-col :span="4">
        <el-cascader
          :props="{ multiple: true }"
          :placeholder="language('LINGJIANLEIXING', '零件类型')"
    ...
    <el-table
      ...
      @selection-change="handleSelectionChangeSupplier"
      height="400px"
      class="margin-top20"
      :data="tableData"
      stripe
    ...
    >
      ...
      >
        <template slot-scope="scope">
          <el-popover
            :disabled="scope.row.partType.length < 2"
            width="427px"
            trigger="hover"
            placement="top"
          >
            ...
            <div v-if="scope.row.partType.length" slot="reference">
              {{
                scope.row.partType.length && scope.row.partType[0].parentType
              }}-{{
                scope.row.partType.length && scope.row.partType[0].partType
              }}
            </div>
          </el-popover>
        </template>
      ...
      >
        <template slot-scope="scope">
          <div
            v-if="
              scope.row.factoryAddress.country ||
              scope.row.factoryAddress.province ||
              scope.row.factoryAddress.city
            "
          >
            <span>{{ scope.row.factoryAddress.country }}</span>
            <span v-if="scope.row.factoryAddress.province"
              >-{{ scope.row.factoryAddress.province }}</span
            >
            <span v-if="scope.row.factoryAddress.city"
              >-{{ scope.row.factoryAddress.city }}</span
            >
          </div>
          <div v-else>
            <span>{{ scope.row.address.country }}</span>
            <span v-if="scope.row.address.province"
              >-{{ scope.row.address.province }}</span
            >
            <span v-if="scope.row.address.city"
              >-{{ scope.row.address.city }}</span
            >
          </div>
        </template>
      </el-table-column>
    ...
    <span slot="footer" class="dialog-footer">
      <iButton @click="creatSupplier">{{
        language('CHUANGJIANXINGONGYINGSHANG', '创建新供应商')
      }}</iButton>
      <iButton @click="addNodeToDag()">{{
        language('QUERENTIANJIA', '确认添加')
      }}</iButton>
    </span>
  </iDialog>
</template>
...
export default {
  ...
  props: {
    ...
    niterFlag: { type: Boolean, default: false }
  },
  data() {
    // 这里存放数据
    return {
  ...
  methods: {
      ...
      this.$refs.ruleForm.validate(async (valid) => {
        if (valid) {
          if (this.selectTable.length === 0) {
            iMessage.warn(
              this.language('QINGXUANZHEYITIAOSHUJU', '请选择一条数据')
            )
          } else if (this.selectTable.length > 1) {
            iMessage.warn(
              this.language('ZHINENGXUANZHEYITIAOSHUJU', '只能选择一条数据')
            )
          } else {
            const pms = {
            ...
            this.resultMessage(res, () => {
              this.clearDiolog()
              if (this.niterFlag) {
                this.$parent.$parent.getCardChain()
              } else {
    ...
    handleAdd() {
      if (this.partForm.partType == '') {
        iMessage.warn(
          this.language('QINGXUANZELINGJIANLEIXING', '请选择零件类型')
        )
        return false
      }
