...
<template>
  <iDialog v-if="value"
           :title="flag==='edit'?language('BJGYS','编辑供应商信息'):language('CHUANGJIANGONGYINGSHANG','创建新供应商')"
           width="90%"
           :visible.sync="value"
           show-close
           @close="clearDiolog">
    <div class="title margin-bottom20">{{language('GONGYINGSHANGXINXI','供应商信息')}} </div>
    <iFormGroup :rules="rules"
                ref="ruleForm"
                :model='supplier'
                row="3">
      ...
      <iFormItem prop="supplierNameCn"
        ...
        <iLabel slot="label"
                :required="true"
                :label="language('GONGYINGSHANGZh','供应商中文名')"></iLabel>
        <iInput :disabled="node.chainSource==='SQO'&&!!disabledSupplier.supplierNameCn"
                :placeholder="language('QINGSHURU','请输入')"
                v-model="supplier.supplierNameCn"></iInput>
      </iFormItem>

      <iFormItem prop="supplierNameEn">
        <iLabel slot="label"
                :required="true"
                :label="language('GONGYINGSHANGEn','供应商英文名')"></iLabel>
        <iInput :disabled="node.chainSource==='SQO'&&!!disabledSupplier.supplierNameEn"
                :placeholder="language('QINGSHURU','请输入')"
                v-model="supplier.supplierNameEn"></iInput>
      </iFormItem>
      ...
      <iFormItem prop="creditCode">
        <div slot="label"
          ...
          <iLabel slot="reference"
                  :required="true"
                  :label="language('TONGYISHEHUIXINGYONGDAIMA','统一社会信用代码')"></iLabel>
          <el-popover width="427"
                      trigger="hover"
                      placement="top-start">
            <span>
              {{language('GSZCHTYSHXYDMSZDSYZGGYSDBTX','工商注册号/统一社会信用代码是针对所有中国供应商的必填项（15或18位）可以在全国组织机构统一社会信用代码公示查询平台')}}
            </span>
            <a class="openLinkText"
               href="http://www.cods.org.cn">http://www.cods.org.cn</a>
        ...
        <iInput :disabled="node.chainSource==='SQO'&&!!disabledSupplier.creditCode"
                :placeholder="language('QINGSHURU','请输入')"
                v-model="supplier.creditCode"></iInput>
      </iFormItem>
      ...
      <iFormItem>
        <div slot="label"
             class="flex">
          <iLabel slot="reference"
                  :label="language('DUNS','DUNS')"></iLabel>
          <el-popover width="427"
                      trigger="hover"
                      placement="top-start">
            <div v-if="flag==='edit'">
              ...
              <a class="openLinkText"
                 href="http://www.dunsregistered.com">http://www.dunsregistered.com</a>
        ...
        <div class="duns flex-align-center">
          <iInput :placeholder="language('QINGSHURU','请输入')"
                  maxlength="2"
                  v-model="supplier.one"></iInput>
          <span></span>
          <iInput :placeholder="language('QINGSHURU','请输入')"
                  maxlength="3"
                  v-model="supplier.two"></iInput>
          <span></span>
          <iInput :placeholder="language('QINGSHURU','请输入')"
                  maxlength="4"
                  v-model="supplier.three"></iInput>
        </div>
      </iFormItem>
      ...
      <iFormItem prop="area">
        <iLabel slot="label"
                :required="true"
                :label="language('GUOJIA_SHENGFEN_CHENSHI','国家-省份-城市')"></iLabel>
        <el-cascader :disabled="node.chainSource==='SQO'&&!!disabledSupplier.area"
      ...
      <iFormItem :label="language('XIANGXIDIZHI','详细地址')">
        <iInput :disabled="node.chainSource==='SQO'&&!!disabledSupplier.address"
                :placeholder="language('QINGSHURU','请输入')"
                v-model="supplier.address"></iInput>
      </iFormItem>

      <iFormItem :label="language('YOUBIANHAO','邮编号')">
        <iInput :placeholder="language('QINGSHURU','请输入')"
                v-model="supplier.postCode"></iInput>
      </iFormItem>

      <iFormItem :label="language('GONGSIXINGZHI','公司性质')">
        <iInput :disabled="node.chainSource==='SQO'&&!!disabledSupplier.enterpriseType"
                :placeholder="language('QINGSHURU','请输入')"
                v-model="supplier.enterpriseType"></iInput>
      </iFormItem>

      <iFormItem :label="language('GONGSIGUIMO','公司规模')">
        <iInput :placeholder="language('QINGSHURU','请输入')"
                v-model="supplier.companySize"></iInput>
      </iFormItem>

      <iFormItem :label="language('ZHUCEZIJING','注册资金')">
        <iInput :placeholder="language('QINGSHURU','请输入')"
                v-model="supplier.registeredCapital"></iInput>
      </iFormItem>

      <iFormItem :label="language('ZIZHIYURENZHENG','资质与认证')">
        <iInput :placeholder="language('QINGSHURU','请输入')"
                v-model="supplier.certification"></iInput>
      </iFormItem>
    ...
    <div class="partType"
      ...
      <!-- 零件类型 -->
      <iFormGroup :rules="rulesPartType"
        ...
        <iFormItem prop="partType">
          <iLabel slot="label"
                  :required="true"
                  :label="language('LINGJIANLEIXING','零件类型')"></iLabel>
          <el-cascader clearable
                       separator="-"
                       v-model="partType.partType"
                       :options="formGroup.partList"></el-cascader>
        </iFormItem>
      ...
      <table-list :height="120"
        ...
        <template #partName="scope">
          <div v-if="scope.row.isEdit">
            <iInput :placeholder="language('QINGSHURU','请输入')"
                    v-model="scope.row.partName"></iInput>
          </div>
        ...
        <template #partNum="scope">
          <div v-if="scope.row.isEdit">
            <iInput :placeholder="language('QINGSHURU','请输入')"
                    v-model="scope.row.partNum"></iInput>
          </div>
    ...
    <iFormGroup :rules="rulesPlant"
      ...
      <iFormItem prop="factoryName">
        <iLabel slot="label"
                :required="true"
                :label="language('GONGCHANGMINGCHENG','工厂名称')"></iLabel>
        <iInput :placeholder="language('QINGSHURU','请输入')"
                v-model="plant.factoryName"></iInput>
      </iFormItem>
      ...
      <iFormItem prop="area">
        <iLabel slot="label"
                :required="true"
                :label="language('GUOJIA_SHENGFEN_CHENSHI','国家-省份-城市')"></iLabel>
        <el-cascader v-model="plant.area"
                     :placeholder="language('QINGXUANZHE','请选择')"
                     :options="formGroup.areaList"
                     :props="{multiple:false}"
                     :clearable="true"
      ...
      <iFormItem :label="language('XIANGXIDIZHI','详细地址')">
        <iInput :placeholder="language('QINGSHURU','请输入')"
                v-model="plant.address"></iInput>
      </iFormItem>

      <iFormItem :label="language('YOUBIANHAO','邮编号')">
        <iInput :placeholder="language('QINGSHURU','请输入')"
                v-model="plant.postCode"></iInput>
      </iFormItem>

      <iFormItem :label="language('GONGCHANGZHANDIMIANJI','工厂占地面积（平方米）')">
        <iInput :disabled="node.chainSource==='SQO'&&!!disabledPlant.areaCovered"
                :placeholder="language('QINGSHURU','请输入')"
                v-model="plant.areaCovered"></iInput>
      </iFormItem>
    </iFormGroup>
    <span slot="footer"
          class="dialog-footer">
      <iButton v-if="flag==='create'"
               @click="handleBack">{{language('FANHUI','返回')}}</iButton>
      <iButton v-if="flag==='create'"
               @click="handleCreate">{{language('QUERENCHUANGJIAN','确认创建')}}</iButton>
      <iButton v-if="flag==='edit'"
               @click="handleSubmit">{{language('QUERENXIUGAI','确认修改')}}</iButton>
    </span>
  </iDialog>
</template>
...
export default {
  ...
  methods: {
    ...
    async querycardChainId (id) {
      ...
      // 工厂
      this.plant = Object.assign(res.data.plant, { 'id': res.data.plant['factoryId'] });
